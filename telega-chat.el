...
(declare-function telega-chat-full-info "telega-info" (chat &optional locally-p))
...
(defun telega-chat--ensure (chat)
  "Ensure CHAT resides in `telega--chats'.
...
(defun telega-chat-by (pred)
  "Find chat by predicate PRED."
  (catch 'found
    (maphash (lambda (_chat-id chat)
               (when (funcall pred chat)
                 (throw 'found chat)))
             telega--chats)))
...
(defun telega-chat-by-supergroup (supergroup)
  ...
  (or
   ...
   (telega-chat-by (lambda (chat)
                     (eq (telega-chat--supergroup-locally chat)
                         supergroup)))))

(defun telega-chat-by-basicgroup (basicgroup)
  "Return chat by BASICGROUP."
  ;; Try very fast heuristic first
  (or (telega-chat-get (- (plist-get basicgroup :id)) 'offline)
      ;; Fallback to full scan
      (telega-chat-by (lambda (chat)
                        (eq (telega-chat--info-locally chat)
                            basicgroup)))))

(defun telega-chat-by-secretchat (secretchat)
  "Return chat by SECRETCHAT."
  (telega-chat-by (lambda (chat)
                    (eq (telega-chat--info-locally chat)
                        secretchat))))
...
(defun telega-chat-by-username (username)
  ...
  (telega-chat-by (lambda (chat)
                    (equal (telega-chat-username chat) username))))

(defun telega-chat-me (&optional can-create-p)
  "Chat with myself, a.k.a Saved Messages.
If CAN-CREATE-P is non-nil, then Saved Messages chat will be created."
  ;; NOTE: Saved Messages has same id as me user
  (or (telega-chat-get telega--me-id 'offline)
      (and can-create-p (telega--createPrivateChat (telega-user-me)))))
...
(defun telega-chat--info-locally (chat)
  (telega-chat--info chat 'locally))
      ...
      (let* ((chat-type (plist-get chat :type))
             ...
             (tc-type (cond ((and (eq type-sym 'supergroup)
                            ...
                            ((and (eq type-sym 'private)
                                  (telega-chat-match-p chat 'bot-user))
                             'bot)
                            (t type-sym))))
...
Combines chat permissions and admin/owner permissions."
  (let ((perms (copy-sequence (cddr (plist-get chat :permissions)))))
    ;; NOTE: `perms' might be nil, i.e. chat's permissions is empty
    ;; for channel creators, thats why we use `setq` to put new props
    ;; into `perms'.  See https://t.me/emacs_telega/49852
      ...
      (cl-case (telega--tl-type status)
        ...
        (chatMemberStatusAdministrator
         (telega--tl-dolist ((pname pval) (plist-get status :rights))
           (setq perms (plist-put perms pname pval))))
...
Format title using FMT-TYPE, one of:
  ...
  `first-name' - Uses only first name (for private/secret chats).
  `last-name' - Uses only last name (for private/secret chats).
...
If NO-BADGES is specified, then do not attach any chat badges at the
  ...
  (let* ((raw-title (or (when (eq fmt-type 'username)
         ...
         (chat-user (telega-chat-user chat))
         (title0 (cond ((and fmt-type chat-user)
                        (telega-user-title chat-user fmt-type 'no-badges))
                       (raw-title
                        raw-title)
                       (chat-user
                        (telega-user-title chat-user 'full-name 'no-badges))
                       (t
                        ;; NOTE: Channels we are banned in can have
                        ;; empty title
         ...
         (title
          (if no-badges
            ...
            (let ((info (telega-chat--info chat 'offline)))
              (concat
               ;; NOTE: Verification Status badges goes first, so chat
               ;; won't trick with fake statuses using emoji status
               ;; icons
               ...
               (when (plist-get info :is_direct_messages_group)
                 (telega-ins--as-string
                  (telega-ins " ")
                  (telega-ins--with-face 'telega-msg-inline-reply
                    (telega-ins-i18n "lng_direct_badge"))))
...
(defun telega-chat-topics (chat)
  ...
  (sort (mapcar #'cdr (telega-chat-topics-alist chat))
        (lambda (t1 t2)
          (cond ((and (plist-get t1 :is_pinned)
                      (not (plist-get t2 :is_pinned)))
                 t1)
                ((and (plist-get t2 :is_pinned)
                      (not (plist-get t1 :is_pinned)))
                 t2)
                (t
...
(defvar telega-chat-button-map
  (let ((map (make-sparse-keymap)))
    ...
    (define-key map (kbd "o") 'telega-transient-chat-operate)
    (define-key map (kbd "r") 'telega-chat-toggle-read)
    (define-key map (kbd "d") 'telega-transient-chat-delete)
...
(defun telega-chat-add-member (chat user &optional forward-limit)
  ...
  (cl-assert user)
  (let ((reply (telega--addChatMember chat user forward-limit)))
    (unless (seq-empty-p (plist-get reply :failed_to_add_members))
      (user-error "telega: Failed to add member"))))
...
(defun telega-describe-chat--inserter (chat)
  ...
  (let ((chat-ava (telega-msg-sender-avatar-image-three-lines chat)))
    ...
    (telega-ins--with-face 'telega-shadow
      (cond ((telega-chat-match-p chat 'is-direct-messages-group)
             (telega-ins-i18n "lng_chat_status_direct"))
            ((telega-chat-match-p chat 'is-broadcast-group)
             (telega-ins-i18n "lng_rights_gigagroup_title"))
    ...
    (when (telega-me-p chat)
      ...
      (telega-ins--ui-button (telega-i18n "lng_settings_upload")
    ...
    (when (telega-chat-match-p chat '(my-permission :can_invite_users))
      (telega-ins " ")
      (telega-ins--ui-button (telega-i18n "lng_profile_add_participant")
        'action (lambda (_button)
                  (call-interactively #'telega-chat-add-member))))
    (when (telega-chat-match-p chat '(my-permission :can_change_info))
      (telega-ins " ")
      (telega-ins--ui-button (telega-i18n "lng_profile_set_group_photo")
        :value chat
    ...
    ;; Archive/Unarchive
    (telega-ins " ")
    (telega-ins--ui-button (if (telega-chat-match-p chat 'archive)
  ...
  (when-let ((price (telega-chat-match-p chat 'paid-message)))
    (telega-ins--help-message
     (telega-ins-i18n "lng_send_charges_stars_text"
  ...
  (when-let ((sender-id (telega-chat-match-p chat 'has-default-sender)))
    (telega-ins-describe-item "Default sender"
      (telega-ins--msg-sender (telega-msg-sender sender-id)
        :with-avatar-p t
        :with-username-p t
        :with-brackets-p t)
      (telega-ins " ")
      (telega-ins--ui-button (telega-i18n "telega_edit")
        :value chat
        :action #'telega-chat-set-default-sender)))
  ...
  (when-let ((theme (plist-get chat :theme)))
    (telega-ins-describe-item "Theme"
  ...
  (when (telega-chat-match-p chat '(type basicgroup supergroup))
    (let ((my-perms (telega-chat-member-my-permissions chat)))
      (telega-ins-describe-item
          (telega-i18n "lng_rights_user_restrictions")
        (telega-ins--line-wrap-prefix "  "
          (dolist (perm-spec telega-chat--chat-permissions)
            ...
            (let ((perm-value (telega--tl-get chat :permissions (car perm-spec))))
              ...
              (let ((active-p (plist-get my-perms :can_restrict_members)))
  ...
  (when (and (listp telega-debug) (memq 'info telega-debug))
    (let ((print-length nil))
      ...
      (when-let ((chat-fi (telega-chat-full-info chat)))
        (telega-ins--with-face 'bold
          (telega-ins "Full-Info: "))
        (telega-ins-fmt "%S" chat-fi)
        (telega-ins "\n"))
      )))
...
(defun telega-describe-chat-similar-channels (chat)
  "Show all chats similar to the given CHAT.
CHAT must be a channel."
  (interactive (list (or telega-chatbuf--chat (telega-chat-at (point)))))
  (with-telega-help-win "*Telegram Similar Channels*"
    (telega-ins--with-face 'telega-describe-section-title
    ...
    (telega--getChatSimilarChats chat
      (telega--gen-ins-continuation-callback 'loading
        (lambda (similar-chats)
          (when (seq-empty-p similar-chats)
            (telega-ins-i18n "lng_profile_similar_channels"
              :count 0))
          (seq-doseq (schat similar-chats)
            (telega-root--global-chat-pp schat)))))))

(defun telega-describe-chat-members (chat)
  "Show CHAT members."
  (interactive (list (or telega-chatbuf--chat (telega-chat-at (point)))))
  (let* ((chat-type (telega-chat--type chat))
    ...
    (with-telega-help-win "*Telegram Chat Members*"
      (telega-ins--with-face 'telega-describe-section-title
        (telega-ins (telega-i18n (if (telega-chat-channel-p chat)
                                     "lng_chat_status_subscribers"
                                   "lng_chat_status_members")
                      :plural-count member-count
                      :count (telega-number-human-readable member-count))
                    ": ")
        (telega-ins--msg-sender chat
          :with-avatar-p t
          :with-username-p t
      ...
      (cond ((eq chat-type 'basicgroup)
             (telega-ins--chat-members
              (append (plist-get full-info :members) nil)))

            ((not (plist-get full-info :can_get_members))
             (telega-ins "Can't get group members"))

            (t
             (cl-assert (memq chat-type '(supergroup channel)))
             (telega--getSupergroupMembers info nil nil nil
               (telega--gen-ins-continuation-callback 'loading
                 #'telega-ins--chat-members))))
      )))
           ...
           (cl-remove-if-not
            (lambda (user)
              (and (plist-get user :is_contact)
                   (not (memq (telega-user-chat user) chats))))
...
(defun telega-chat-join-by-link (link)
  ...
  (when-let ((chat (telega--joinChatByInviteLink link)))
...
(defun telega-chat-set-default-sender (chat)
  "Set default sender for the CHAT."
  (interactive (list (or telega-chatbuf--chat (telega-chat-at (point)))))
  (telega--setChatMessageSender chat
    (telega-completing-read-msg-sender
     "Default sender: "
     (or (telega--getChatAvailableMessageSenders chat)
...
Return newly created chat."
  ...
  (cond ((string= chat-type "basicgroup")
         (let* ((title (read-string "Chat Title: "))
           ...
           (telega--createNewBasicGroupChat title users
             (lambda (group)
               (let ((chat (telega-chat-get (plist-get group :chat_id)))
                     (failed-users (telega--tl-get group :failed_to_add_members
                                                   :failed_to_add_members)))
                 (unless (seq-empty-p failed-users)
                   (plist-put chat :telega-failed-to-add-users
                              (mapcar (lambda (failed-user)
                                        (telega-user-get
                                         (plist-get failed-user :user_id)))
                                      failed-users)))
                 (telega-chat--pop-to-buffer chat))
               ))))
...
(defun telega-chat-set-description (chat descr)
  ...
  (interactive
   (let* ((chat (or telega-chatbuf--chat (telega-chat-at (point))))
          ...
          (full-info (telega-chat-full-info chat)))
     (list chat
           (read-string (concat (telega-i18n "lng_info_about_label") ":")
...
keep the point, where it is."
  (interactive "P")
  (telega-chat--pop-to-buffer (telega-chat-me 'can-create))
  (when arg
    (goto-char (point-max))))
...
multiple chats are important."
  (interactive
   (list (let ((ichats (telega-filter-chats (telega-chats-list)
                         'important)))
           (cond ((null ichats)
                 ...
                 (t
                  (telega-completing-read-chat
                   "Important Chat: " ichats
                   telega-chat-switch-important-sort-criteria))))))
...
CHAT considered unread if matches `telega-unread-chat-temex'."
  (interactive
   (list (or (car (sort (telega-filter-chats (telega-chats-list)
...
;; ** telega-chat-preview-mode
;;
;; To open
;; - {{{user-option(telega-chat-preview-mode-from-last-message, 2)}}}
...
(defvar telega-chatbuf-fastnav-map
  (let ((map (make-sparse-keymap)))
    ...
    ;;; ellit-org: chatbuf-fastnav-bindings
    ;; - {{{where-is(telega-chatbuf-next-outgoing,telega-chat-mode-map)}}} ::
    ;;   {{{fundoc(telega-chatbuf-next-outgoing, 2)}}}
    (define-key map (kbd "o") 'telega-chatbuf-next-outgoing)
...
(defvar telega-chat-mode-map
  (let ((map (make-sparse-keymap)))
    ...
    ;;; ellit-org: chatbuf-attach-bindings
    ;; - {{{where-is(telega-transient-chatbuf-input-options,telega-chat-mode-map)}}} ::
    ;;   {{{fundoc(telega-transient-chatbuf-input-options,2)}}}
    (define-key map (kbd "C-c C-o") #'telega-transient-chatbuf-input-options)
...
(define-button-type 'telega-prompt
  ...
  ;; Mark button as inactive to make `telega-button-forward' ignore it
  'inactive t
...
Take threads and topics into account."
  (or (when telega-chatbuf--topic
        (or
         ;; messages thread
         (telega--tl-get telega-chatbuf--topic :reply_info :last_message_id)
         ;; topic
         (telega--tl-get telega-chatbuf--topic :last_message :id)))
...
Takes into account `telega-chatbuf--topic'."
  (cond ((telega-topic-match-p telega-chatbuf--topic '(type thread))
        ...
        ((telega-topic-match-p telega-chatbuf--topic '(type sm))
         ...
         ;; `:last_read_inbox_message_id', we use 0 in this case so
         ;; `telega-chatbuf-next-unread' will work if topic filter is
         ;; applied
         0)
  ...
  (let ((album-id (plist-get msg :media_album_id))
    ...
    (unless (telega-zerop album-id)
      (unless node
        (setq node (telega-chatbuf--node-by-msg-id (plist-get msg :id))))
      (when node
        (setq album-messages (list msg))
        ;; Traverse backward
        (let ((prev-node (ewoc-prev telega-chatbuf--ewoc node)))
          (while prev-node
            (let ((prev-msg (ewoc-data prev-node)))
              (if (equal album-id (plist-get prev-msg :media_album_id))
                  (progn
                    (setq album-messages (cons prev-msg album-messages))
                    (setq prev-node (ewoc-prev telega-chatbuf--ewoc prev-node)))
                (setq prev-node nil)))))
        ;; Traverse forward
        (let ((next-node (ewoc-next telega-chatbuf--ewoc node)))
          (while next-node
            (let ((next-msg (ewoc-data next-node)))
              (if (equal album-id (plist-get next-msg :media_album_id))
                  (progn
                    (setq album-messages (append album-messages (list next-msg)))
                    (setq next-node (ewoc-next telega-chatbuf--ewoc next-node)))
                (setq next-node nil)))))))
    album-messages))
  ...
  (when (and (telega-msg-from-history-p msg)
    ...
    (telega--viewMessages telega-chatbuf--chat (list msg)
      :source (cond (telega-chat-preview-mode
                    ...
                    (telega-chatbuf--topic
                     (cl-ecase (telega--tl-type telega-chatbuf--topic)
                       (messageThreadInfo
      ...
      ;; NOTE: TDLib does not yet sends updates for forum topic's
      ;; `:unread_count', so request a topic update explicitly after
      ;; viewing a message.
      ;; Same thing for the messageThreadinfo, TDLib has no update
      ;; events for its `:unread_message_count', so request thread
      ;; info update explicitly
      :callback
      (cond ((telega-topic-match-p telega-chatbuf--topic '(type forum))
             (let ((topic-id (telega-topic-id telega-chatbuf--topic)))
            ...
            ;; `:unread_message_count', so commented out
            ((telega-topic-match-p telega-chatbuf--topic '(type thread))
             (let ((msg-id (telega-topic-id telega-chatbuf--topic)))
               (lambda-with-current-buffer (_ignored)
                 (telega--getMessageThread telega-chatbuf--chat msg-id
                   (lambda-with-current-buffer (thread-info)
                     (setq telega-chatbuf--topic thread-info)
                     (telega-chatbuf--chat-update "topic-update"))))))
            )
      )))
    ...
    (telega-ins--as-string
     ...
     (telega-ins--with-face 'bold
       (let ((title (plist-get telega-chatbuf--msg-filter :title)))
         (telega-ins (if (functionp title) (funcall title) title))))
     (when-let ((sender (plist-get telega-chatbuf--msg-filter :sender)))
       ...
       (telega-ins--raw-button
         ...
         (telega-ins--msg-sender sender
           ...
           :with-username-p t
           :with-brackets-p t)))
     ;; NOTE: total number of messages will be displayed by
...
(defun telega-chatbuf-header-messages-count ()
  "Formatter for the number of found/total/unread messages."
  (telega-chatbuf--dirtiness-init
   "updateChatReadInbox"                ;unread_count
   ;; For a number of comments in the discussion group
   "updateMessageInteractionInfo"
   ;; Number of unread messages in a chatbuf's topic
   "topic-update"
   ;; Number of total messages found by inplace search
   "highlight-text"
   )

  (cond (telega-chatbuf--msg-filter
         (when-let ((msg-count (plist-get telega-chatbuf--msg-filter
                                          :total-count)))
           (telega-i18n "lng_search_found_results"
             :count msg-count)))

        ((and telega-chatbuf--inplace-search-filter
              telega-highlight-text-mode
              (get-text-property 0 :telega-inplace-query
                                 telega-highlight-text-regexp))
         (when-let ((msg-count (plist-get telega-chatbuf--inplace-search-filter
                                          :total-count)))
           (telega-i18n "lng_search_found_results"
             :count msg-count)))
        ...
        ((telega-topic-match-p telega-chatbuf--topic '(type sm dm))
         (when-let ((msg-count (plist-get telega-chatbuf--topic
                                          :telega_message_count)))
           (telega-i18n "lng_forum_messages"
             :plural-count msg-count
             :count (telega-number-human-readable msg-count))))

        (t
         ;; No filtering is applied, show number of unread messages in
         ;; the chat/topic/thread
...
(defun telega-chatbuf-header-topic (&optional max-width)
...
(defun telega-ins--chat-action-bar-button (chat action-bar)
  ...
  (cl-ecase (telega--tl-type action-bar)
    (chatActionBarReportSpam
     (telega-ins--ui-button (if (telega-chat-match-p chat
    ...
    (chatActionBarReportAddBlock
     (telega-ins--ui-button (telega-i18n "lng_new_contact_block")
       :passive-face '(telega-red telega-box-button)
       :value chat
       :action #'telega-transient-chat-action-report-block)
     (telega-ins " ")
    ...
    (chatActionBarSharePhoneNumber
     (telega-ins--ui-button (telega-i18n "lng_new_contact_share")
...
(defun telega-chatbuf-footer-ins-active-stories ()
  "Inserter for the active stories in the chatbuf's footer."
  (telega-chatbuf--dirtiness-init
   "active-stories"
  ...
  (when-let* ((match-result
    ...
    (unless (seq-empty-p stories)
      ...
      (when (> (length stories) show-nstories)
        ...
        (telega-ins--ui-button (telega-i18n "lng_stories_show_more")
          'face 'telega-link
          'action (lambda (_ignored)
                    (plist-put telega-chatbuf--hidden-headers
...
(defun telega-chatbuf-footer-ins-pinned-stories ()
  "Inserter for the pinned stories in the chatbuf's footer."
  (telega-chatbuf--dirtiness-init
   "pinned-stories"
...
(defun telega-chatbuf-footer-ins-active-vvnote ()
  "Inserter for the active voice/video note currently playing in the chatbuf."
...
(defun telega-chatbuf-footer-ins-active-video-chat ()
  "Inserter for the active video chat in the chatbuf's footer."
  (telega-chatbuf--dirtiness-init
   "group-call"
  ...
  (when-let* ((video-chat (plist-get telega-chatbuf--chat :video_chat))
    ...
    (telega-ins (telega-i18n "lng_group_call_title")
                (when (plist-get group-call :is_rtmp_stream)
                  (propertize " (rtmp)" 'face 'telega-shadow))
                ": "
    ...
    (when (> (plist-get group-call :scheduled_start_date) 0)
      ...
      ;; Start Now
      (telega-ins " ")
      (telega-ins--ui-button (telega-i18n "lng_group_call_start_now")
        :value group-call
    ...
    (when (and (plist-get video-chat :has_participants)
      ...
      (telega-ins " ")
      (telega-ins--ui-button (telega-i18n "telega_show")
        :value group-call
        :action #'telega-describe-group-call)
      (telega-ins "\n"))))

(defun telega-chatbuf-footer-ins-invite-failed-to-add-users ()
  "Inserter for the list of users that forbids direct add to the group.
These users can be added to group only via invite link."
  ;; List of users that restricts adding them to the group
  (when-let ((failed-to-add-users (plist-get telega-chatbuf--chat
                                             :telega-failed-to-add-users))
    ...
    (telega-ins--text-button (telega-symbol 'button-close)
      ...
      'action (lambda (_ignored)
                (plist-put telega-chatbuf--chat
                           :telega-failed-to-add-users nil)
                (telega-chatbuf--footer-update)))
    (telega-ins " ")
    (telega-ins-i18n "lng_via_link_group_many"
      :count (length failed-to-add-users))
    (telega-ins "\n")
    (seq-doseq (user failed-to-add-users)
      (telega-ins "  ")
        ...
        (let ((column1 (/ (+ telega-chat-fill-column 10 1) 2)))
          (telega-ins--with-attrs (list :min column1
            ...
            (telega-ins--msg-sender user
              ...
              :with-brackets-p t))))
      (telega-ins " ")
      (telega-ins--ui-button (telega-i18n "lng_via_link_send")
        :value user
        :action #'telega-chatbuf-invite-user-via-link))
    (telega-ins "\n")))

(defun telega-chatbuf-footer-ins-auto-delete-messages ()
...
(defun telega-chatbuf-footer-ins-reply-markup-buttons ()
  "Inserter for the reply markup buttons in the chatbuf's footer."
  (telega-chatbuf--dirtiness-init "updateChatReplyMarkup")

  ;; NOTE: do not show reply markup unless we can send messages to bot
...
(defun telega-chatbuf-footer-ins-restriction-reason ()
  "Inserter for the chat restriction reasons in the chatbuf's footer."
  ;; User's restriction reason
  (when-let* ((my-status (telega-chat-member-my-status telega-chatbuf--chat))
              ((eq 'chatMemberStatusRestricted (telega--tl-type my-status)))
              (my-perms (plist-get my-status :permissions)))
    (telega-ins--with-face 'telega-shadow
      (cond ((not (plist-get my-perms :can_send_basic_messages))
             (telega-ins (telega-i18n "lng_restricted_send_message") "\n"))
            ((not (plist-get my-perms :can_send_polls))
             (telega-ins (telega-i18n "lng_restricted_send_polls") "\n"))
            ((not (seq-every-p (lambda (media-perm)
                                 (plist-get my-perms (car media-perm)))
                               telega-chat--chat-media-permissions))
             (telega-ins (telega-i18n "lng_restricted_send_media") "\n"))
            (t
             ;; Looks like status is restricted, but everything is allowed
             ;(telega-ins (telega-i18n "lng_restricted_send_media") "\n")
             ))))

  ;; Chat's restriction reason
  (when-let ((reason (telega-tl-str
                      (telega-chatbuf-match-p '(info :restriction_info))
                      :restriction_reason)))
    (telega-ins--with-face 'bold
...
(defun telega-chat-toggle-payment-confirm (chat)
  "Toggle payment confirmation for the messages in the CHAT."
  (setf (telega-chat-uaprop chat :payment-confirm)
        (if (telega-zerop (telega-chat-uaprop chat :payment-confirm))
            1
          0))
  (telega-chatbuf--chat-update "paid-message"))

(defun telega-chatbuf-footer-ins-paid-message-star-count ()
  "Inserter for paid messages."
  (telega-chatbuf--dirtiness-init
   "paid-message" "updateSupergroup" "updateUser")

  (when-let ((price (telega-chat-match-p telega-chatbuf--chat 'paid-message)))
    (telega-ins--text-button
        (if (telega-zerop
             (telega-chat-uaprop telega-chatbuf--chat :payment-confirm))
            (telega-symbol 'checkbox-off)
          (telega-symbol 'checkbox-on))
      'face 'telega-link
      'help-echo (telega-i18n "lng_payment_confirm_dont_ask")
      :value telega-chatbuf--chat
      :action #'telega-chat-toggle-payment-confirm)
    (telega-ins " ")
    (telega-ins-i18n "lng_message_paid_ph"
      :amount (telega-ins--as-string
               (telega-ins (telega-symbol 'telegram-star))
               (telega-ins-fmt "%d" price)))
    (telega-ins "\n")))

(cl-defun telega-chatbuf-footer-ins-bot-info (&key (with-menu-p t))
  "Inserter for the bot description if history is empty."
  (telega-chatbuf--dirtiness-init "updateUserFullInfo")

  (when (telega-chat-bot-p telega-chatbuf--chat)
    (when-let* ((ci (telega-chat--info telega-chatbuf--chat t))
                (fi (telega--full-info ci))
                (bi (plist-get fi :bot_info)))
      (unless (plist-get telega-chatbuf--chat :last_message)
        (when-let ((photo (plist-get bi :photo)))
          (telega-ins--photo photo)
          (telega-ins "\n"))
        (when-let ((anim (plist-get bi :animation)))
          (telega-ins--animation-image anim 'sliced)
          (telega-ins "\n"))
        (when-let ((desc (telega-tl-str bi :description)))
          (telega-ins--with-face 'bold
          ...
          (telega-ins "\n")))
      (when with-menu-p
        (when-let ((bot-menu (plist-get bi :menu_button)))
          (telega-ins--line-wrap-prefix
              (cons (concat (telega-i18n "lng_bot_menu_button") ": ") nil)
            (telega-ins--bot-menu-button bot-menu))
          (telega-ins "\n")))
      )))

;; TODO: emulate typing
(defvar-local telega-chatbuf--pending-msg-offset nil)
(defvar-local telega-chatbuf--pending-msg-marker nil)
(defvar-local telega-chatbuf--pending-msg-timer nil)

(defun telega-chatbuf-footer-ins-pending-message ()
  "Inserter for the chat's pending message."
  (telega-chatbuf--dirtiness-init
  ...
  (when-let* ((pending-msg
    ...
    (let* ((sender (telega-chat-user telega-chatbuf--chat))
           ...
           (gaps-workaround-p (telega-chat-match-p telega-chatbuf--chat
                                telega-avatar-workaround-gaps-for))
           (content-wrap (make-string awidth ?\s))
           (header-prefix
            (telega-ins--as-string
             (telega-ins--image
...
(defun telega-chatbuf-footer-ins-sponsored-messages ()
  "Inserter for the sponsored messages."
  ...
  (when-let ((sponsored-messages
    ...
    (seq-doseq (sponsored-msg (plist-get sponsored-messages :messages))
      (telega-ins--message
          (telega-msg-create-internal telega-chatbuf--chat
            (telega-fmt-text
...
(defun telega-chatbuf-footer-ins-prompt-delim (&optional with-actions-p
                                                         with-loading-p)
  ...
  (let* ((column telega-chat-fill-column)
         ...
         (actions (when with-actions-p
                    (telega-chat--actions
                     telega-chatbuf--chat (telega-chatbuf--MessageTopic)))))
...
(defun telega-chatbuf-footer-ins-join-button ()
  ...
  (telega-chatbuf--dirtiness-init
   ...
   ;; 'me-is-member or/and 'me-is-owner
   )

  (when (telega-ins--chat-join-button telega-chatbuf--chat
                                      telega-chatbuf--bot-start-parameter)
    (telega-ins "\n")))

(cl-defun telega-chatbuf-footer-ins-aux-plist (&key one-line-p use-symbols-p
                                                    (with-avatar-p t))
  "Insert aux prompt defined by `telega-chatbuf--aux-plist'.
Non-nil ONE-LINE-P to use one line version.
Non-nil USE-SYMBOLS-P to use `reply' and `reply-quote'."
  (telega-chatbuf--dirtiness-init "aux-plist")

  ;; NOTE: In direct messages chat, admin can only write into a topic,
  ;; i.e. reply to a message
  (when (and (not telega-chatbuf--aux-plist)
             (telega-chatbuf-match-p
              '(and is-direct-messages-group
                    (info :is_administered_direct_messages_group))))
    (telega-ins--with-face 'telega-shadow
      (telega-ins (telega-i18n "lng_monoforum_choose_to_reply") "\n")))
  ...
  (when telega-chatbuf--aux-plist
    ...
    (cl-ecase (plist-get telega-chatbuf--aux-plist :aux-type)
      (edit
       (let* ((ccol (unless one-line-p (telega-window-current-column)))
              (aux-msg (plist-get telega-chatbuf--aux-plist :aux-msg))
              (telega-palette-context 'edit)
              (palette (telega-msg-sender-palette (telega-msg-sender aux-msg)))
              (bar-face (append (assq :foreground palette)
                                (assq :background palette)))
              (msg-face (telega-face-with-palette 'telega-chat-prompt-aux
                            palette :background)))
         (if ccol
             (telega-ins--move-to-column (1+ ccol))
           (telega-ins " "))
         (telega-ins--with-face bar-face
           (telega-ins (telega-symbol 'vbar-left)))
         (telega-ins--with-attrs (list :max (- telega-chat-fill-column
                                               (telega-current-column))
                                       :elide t
                                       :face msg-face)
           (telega-ins--with-face 'telega-shadow
             (telega-ins (telega-i18n "lng_edit_message")
                         (when one-line-p ": ")))
           (when one-line-p
      ...
      (reply
       (let* ((ccol (unless one-line-p (telega-window-current-column)))
              ...
              (aux-quote (plist-get telega-chatbuf--aux-plist
                                    :aux-reply-quote))
              (msg-sender (telega-msg-sender aux-msg))
              (telega-palette-context 'reply)
              (palette (telega-msg-sender-palette msg-sender))
              (bar-face (append (assq :foreground palette)
                                (assq :background palette)))
              (msg-face (telega-face-with-palette 'telega-chat-prompt-aux
                            palette :background)))
         ...
         (telega-ins--with-attrs (list :max (- telega-chat-fill-column
           ...
           (telega-ins--with-face 'telega-shadow
             (if use-symbols-p
                 (telega-ins (telega-symbol (if aux-quote 'reply-quote 'reply))
                             " ")
               (telega-ins-i18n (if aux-quote
           ...
           (telega-ins--with-face (telega-msg-sender-title-faces msg-sender)
             (unless (when one-line-p
                       (telega-ins
                        (telega-msg-sender-username msg-sender 'with-@)))
               (telega-ins--msg-sender msg-sender
                 :with-badges-p nil)))
           (telega-ins--aux-msg-topic-one-line aux-msg
             :prefix (telega-symbol 'right-arrow))
           ...
           (when one-line-p
             ...
             (if aux-quote
                 (progn
                   (when (telega--tl-get aux-msg :content :caption)
                     (telega-ins--content-media-thumbnail-one-line aux-msg)
                     (telega-ins " "))
...
(cl-defun telega-chatbuf-footer-ins-input-options ()
  ...
  (telega-chatbuf--dirtiness-init "input-options")

  (when telega-chatbuf--input-options-plist
    (telega-ins--text-button (telega-symbol 'button-close)
      'face 'telega-link
      'action (lambda (_ignored)
                (telega-chatbuf-cancel-input-options))
      'help-echo "Cancel input options")

    (telega-ins "TODO: input options")
    (telega-ins "\n")))

(defvar telega-chatbuf-footer-mode-inserters nil
  "List of inserters for minor modes.")

(defun telega-chatbuf-footer-ins-modes ()
  (seq-some #'funcall telega-chatbuf-footer-mode-inserters))

(defun telega-chatbuf--footer-update ()
  "Redisplay chatbuf's footer."
  (let ((footer (telega-ins--as-string
                 (telega-ins--with-props '(read-only t front-sticky t)
                   (telega-ins--insexp telega-chat-footer-insexp)))))
    ...
    (with-telega-buffer-modify
     (if (< (ewoc-location (ewoc--footer telega-chatbuf--ewoc))
         ...
         (telega-save-cursor
           (telega-ewoc--set-footer telega-chatbuf--ewoc footer))
       (save-excursion
         (telega-ewoc--set-footer telega-chatbuf--ewoc footer))))))
    ...
    (when-let* ((pos (if (eq dir 'entered) (point) oldpos))
      ...
      (unless (telega-msg-internal-p msg)
        (if (eq dir 'entered)
          ...
          ;; NOTE:
          ;; - Hide spoilers when point moves out of message
          ;; - Hide revealed ignored messages
          (when (or (when (plist-get msg :telega-text-spoiler-removed)
                    ...
                    (when (plist-get msg :telega-media-spoiler-removed)
                      ...
                      t)
                    (when (plist-get msg :telega-ignored-revealed-p)
                      (plist-put msg :telega-ignored-revealed-p nil)
                      t))
...
(defun telega-chatbuf-msg--pp-inserter (msg)
  ...
  (cond ((telega-msg-match-p msg
        ...
        ((and (telega-msg-match-p msg `(chat ,telega-chat-group-messages-for))
              (> (point) 3)
              (when-let ((prev-msg (telega-msg-at (- (point) 2)))
                         (trim-regexp (rx (1+ (or " " "\n")))))
                ...
                (and (not (telega-msg-internal-p prev-msg))
                     ...
                     (string-prefix-p
                      ...
                      (string-trim-right
                       (telega-ins--as-string
                        (telega-ins--message-header prev-msg))
                       trim-regexp)))))
         #'telega-ins--message-no-header)

        (t
         telega-inserter-for-msg-button)))

(defun telega-chatbuf-msg--pp (msg &optional for-preview-p)
  "Pretty printer for MSG button inserted in a chatbuf."
  (when-let ((msg-inserter (telega-chatbuf-msg--pp-inserter msg)))
...
\\{telega-chat-mode-map}"
  ...
  ;; To make `visual-fill-column-mode RET' to work out-of-box
  ...
  (when telega-chat-header-line-underline
    (face-remap-add-relative 'header-line :underline '(:position descent)))
  ...
  (let ((telega-chatbuf--dirtiness-symbol 'telega-chatbuf--dirtiness-prompt))
    ;; NOTE: updating group info, position and blocklist affects
    ;; `can-send-or-post' chat temex and updating message sender
    ;; affects `has-default-sender' used in the
    ;; `telega-chat-prompt-insexp'
    (telega-chatbuf--dirtiness-init
     "updateChatPosition" "updateChatBlockList"
     "updateSupergroup" "updateBasicGroup"
     "updateChatPermissions" "updateChatMessageSender")
    (telega-chatbuf--prompt-update))

  ;; For forum chats, load topics unless they are loaded
...
(defun telega-chatbuf-replying-imr ()
  ...
  (when (eq 'reply (plist-get telega-chatbuf--aux-plist :aux-type))
    (let ((aux-msg (plist-get telega-chatbuf--aux-plist :aux-msg))
          (aux-quote (plist-get telega-chatbuf--aux-plist :aux-reply-quote)))
      (if (= (plist-get telega-chatbuf--chat :id)
             (plist-get aux-msg :chat_id))
          (list :@type "inputMessageReplyToMessage"
                :message_id (plist-get aux-msg :id)
                :quote aux-quote)
        (list :@type "inputMessageReplyToExternalMessage"
              :chat_id (plist-get aux-msg :chat_id)
              :message_id (plist-get aux-msg :id)
              :quote aux-quote)))))
...
(defun telega-chatbuf-prompt-ins-body ()
  ...
  (telega-chatbuf--dirtiness-init
   "topic"                             ; telega-chatbuf--topic-thread-msg
  ...
  (telega-ins
   (cond ((telega-chatbuf-match-p 'me-is-anonymous)
          (string-trim-right
           (telega-i18n "lng_send_anonymous_ph") "\\.+"))

         ((and (telega-chatbuf--topic-thread-msg)
               (not (telega-chatbuf-match-p 'me-is-member)))
          (telega-i18n "telega_comment"))

         ((telega-chatbuf-match-p '(and (type channel)
                                        (my-permission :can_post_messages)))
          (string-trim-right
           (telega-i18n (if (telega-chatbuf-match-p
                             '(prop :default_disable_notification))
...
(defun telega-chatbuf-prompt-ins-topic (&optional max-width with-topic-title-p)
  ...
  (when (telega-topic-match-p telega-chatbuf--topic
    ...
    (telega-ins--with-attrs (list :max max-width :align 'left :elide t
      ...
      (telega-ins--topic-icon telega-chatbuf--topic)
...
otherwise set draft only if chatbuf input is also draft."
  (let ((draft-msg (plist-get (or telega-chatbuf--topic telega-chatbuf--chat)
                              :draft_message)))
             ...
             (unless (eq (plist-get (telega-chatbuf-replying-msg) :id)
               ...
               (telega-msg-get telega-chatbuf--chat reply-msg-id
                 (lambda (msg &optional _ignored)
                   (save-excursion
                     (telega-msg-reply msg nil (plist-get reply-to :quote))))))
...
(defun telega-chatbuf--load-initial-history ()
  ...
  (cond ((and telega-chat-preview-mode
        ...
        ;; NOTE: if no messages in thread are read, then insert thread
        ;; starter messages
        ((when-let* ((thread-info (telega-topic-match-p telega-chatbuf--topic
                                    '(type thread)))
                     (thread-last-read-msg-id
                      (telega--tl-get thread-info
                                      :reply_info :last_read_inbox_message_id))
                     (thread-messages
                      (plist-get telega-chatbuf--topic :messages)))
           (memq (telega--tl-get thread-info
                                 :reply_info :last_read_inbox_message_id)
                 (cons 0 (mapcar (telega--tl-prop :id) thread-messages))))
...
If NO-HISTORY-LOAD is specified, do not try to load history."
  (or (with-telega-chatbuf chat
        (current-buffer))
      (with-current-buffer (generate-new-buffer (telega-chatbuf--name chat))
        ...
        (when (plist-get (or telega-chatbuf--topic telega-chatbuf--chat)
                         :draft_message)
...
(defun telega-chatbuf--older-history-loaded ()
  ...
  (when (telega-chatbuf--need-older-history-p)
    ...
    ;; Insert thread starter messages, because it is not included in
    ;; the results of the `getMessageThreadHistory' call
    (when-let* ((thread-info (telega-topic-match-p telega-chatbuf--topic
                               '(type thread)))
                (thread-messages (plist-get telega-chatbuf--topic :messages)))
      (let ((first-msg (telega-chatbuf--first-msg)))
        (when (or (null first-msg)
                  (not (telega-msg-id= (seq-first thread-messages) first-msg)))
          (let ((node (telega-chatbuf--insert-messages thread-messages 'prepend)))
            (telega-chatbuf--manage-discussion-bar node))
...

;;; chatbuf mode-line formatters
(defvar telega-chat-mode-line-string ""
  "Used to cache formatted modeline string for a chatbuf.")
(make-variable-buffer-local 'telega-chat-mode-line-string)
...
(defun telega-chatbuf-mode-line-direct-messages ()
  "Show [Direct messages] button in the chat buffer modeline."
  (when (telega-chatbuf-match-p 'has-direct-messages-group)
    (telega-ins--as-string
     (telega-ins--with-props
         (list 'local-map (eval-when-compile
                            (make-mode-line-mouse-map
                             'mouse-1 #'telega-chatbuf-goto-direct-messages-chat))
               'mouse-face 'mode-line-highlight
               'help-echo (telega-i18n "lng_send_free_channel"
                            :channel (telega-chat-title telega-chatbuf--chat)))
  ...
  (let ((member-count
    ...
    (unless (zerop member-count)
      (if (not use-icons-p)
        ...
        (concat
         (telega-number-human-readable member-count)
...
topic or thread filtering."
  ...
  (when-let* ((no-topic-p (not telega-chatbuf--topic))
...
(defun telega-chatbuf-mode-line-topic (&optional max-width)
  ...
  (when-let ((topic (telega-topic-match-p telega-chatbuf--topic
                      '(type forum sm dm))))
...
(defun telega-chatbuf-mode-line-messages-filter ()
  ...
  (telega-chatbuf--dirtiness-init "msg-filter")
...
(defun telega-chatbuf--header-line-update ()
  ...
  (when telega-chat-header-line-format
    ;; NOTE: If header line becomes visible/hidden it causes contents
    ;; of the chatbuf to shift by one-line, making it visually "jump".
    ;; To prevent this, we scroll window vertically to compensate this
    ;; "jump".
    (let ((old-hl-format header-line-format)
          (cb-win (get-buffer-window)))
      (setq header-line-format
            (telega-format-mode-line telega-chat-header-line-format 'header-line))
      (when cb-win
        (when-let ((nlines (cond ((and old-hl-format (not header-line-format))
                                  -1)
                                 ((and (not old-hl-format) header-line-format)
                                  1)))
                   (win-start (window-start cb-win)))
          ;; NOTE: `set-window-vscroll' does not work if max point is
          ;; visible in the window, thats why we adjust `window-start'
          ;; position instead.  Also, note, that setting window start
          ;; only takes effect on redisplay, thats why we call
          ;; `force-mode-line-update' after modifying window's start
          ;; position
          (save-excursion
            (goto-char win-start)
            (forward-line nlines)
            (set-window-start cb-win (point) 'noforce))))
         ...
         (last-sent-msg
          (if (and backward
            ...
            (telega-chatbuf--next-msg (or edit-msg last-msg)
              msg-temex backward))))
    ;; NOTE: save pending input
    ;; See https://github.com/zevlg/telega.el/issues/336
    (unless edit-msg
      (setq telega-chatbuf--input-idx nil
            telega-chatbuf--input-pending (telega-chatbuf-input-string)))
    ...
    (if last-sent-msg
      ...
      (if (and edit-msg (not backward))
          (progn
            ...
            ;; Restore previously saved pending input
            (when telega-chatbuf--input-pending
              (goto-char (point-max))
              (insert telega-chatbuf--input-pending)))
...
(defun telega-chatbuf-beginning-of-thing (&optional arg)
  ...
  (when (> arg 0)
    (when-let ((msg-start (previous-single-property-change
                           (point) :message-content)))
      (goto-char msg-start))))
    ...
    (with-telega-buffer-modify
     (telega-save-window-start (button-start msg-button) (button-end msg-button)
       (if (eq (telega-msg-at (point)) (ewoc--node-data node))
           (telega-save-cursor
             ;; (telega-ewoc--move-node telega-chatbuf--ewoc node node 'save-point)
...
(defun telega-chatbuf--manage-discussion-bar (node &optional _msg)
  "Manages discussion started bar after NODE.
Always return ewoc node, even if discussion bar is not inserted."
  (when-let ((thread-msg (telega-chatbuf--topic-thread-msg))
             (node-msg (ewoc--node-data node)))
    (when (telega-msg-id= thread-msg node-msg)
...
(defun telega-chatbuf--manage-date-break-bar (node msg)
  "Manages date break bar after NODE before inserting MSG after NODE.
Always return ewoc node, even if date break is not inserted."
  (let ((node-msg (ewoc--node-data node)))
    (when (and telega-chat-use-date-breaks
      ...
      (setq node (ewoc-enter-after
                  ...
                  (telega-msg-create-internal telega-chatbuf--chat
                    (telega-fmt-text
                     (telega-ins--as-string
                      (telega-ins--date (plist-get msg :date) 'date-break-bar))
                     '(:@type "textEntityTypeBold")))))))
  node)
       ...
       (unwind-protect
           (progn
             (seq-doseq (msg messages)
               ;; NOTE: internal messages should not run any hooks
               ;; See https://github.com/zevlg/telega.el/issues/463
               (unless (telega-msg-internal-p msg)
               ...
               ;; Maybe insert "discussion started" bar, such as
               ;; -----(Discussion started)-----
               (setq node (telega-chatbuf--manage-discussion-bar node msg))

               ;; Maybe insert date break, such as
               ;; -----(28 December 2020)-----
               (setq node (telega-chatbuf--manage-date-break-bar node msg))
               ...
               (unless (or (telega-msg-internal-p msg)
                           (plist-get msg :sending_state))
                 (run-hook-with-args 'telega-chatbuf-post-msg-insert-hook msg)))

             ;; NOTE: prepending messages might affect look of the
             ;; following message (with same album-id) or might
             ;; require to insert date brake
             (when-let* ((next-node (ewoc-next telega-chatbuf--ewoc node))
                         (next-msg (ewoc-data next-node)))
               (telega-chatbuf--manage-date-break-bar node next-msg)

               ;; NOTE: `saved-point' might be inside next msg button,
               ;; causing point moving when invalidating content, this
               ;; is undesirable behaviour, so redisplay only if
               ;; `saved-point' is outside that region
               (when-let ((next-msg-button
                           (button-at (ewoc-location next-node))))
                 (unless (or (eq (button-get next-msg-button :inserter)
                                 (telega-chatbuf-msg--pp-inserter next-msg))
                             (<= (button-start next-msg-button)
                                 saved-point
                                 (button-end next-msg-button)))
  ...
  (let ((unread-bar-node (telega-ewoc--find-if telega-chatbuf--ewoc
    ...
    (with-telega-buffer-modify
       ...
       (cond ((and unread-bar-node node)
              (telega-ewoc--move-node
               telega-chatbuf--ewoc unread-bar-node node)
              (save-excursion
                (ewoc-invalidate telega-chatbuf--ewoc node)))
             (unread-bar-node
              (telega-save-excursion
                (ewoc-delete telega-chatbuf--ewoc unread-bar-node)))
             ;; NOTE: Do not insert bar if UNREAD-MSG is the last
             ;; message in the chat, to avoid "Unread Messages" bar
             ;; flickering
             ((and node (not (telega-msg-match-p unread-msg 'is-last)))
              (telega-save-excursion
                (ewoc-enter-before
                 telega-chatbuf--ewoc
                 node
    ...
    (cond ((and (telega-chatbuf--history-state-get :newer-freezed)
          ...
          (t
           (and (telega-chatbuf--filter-match-msg-p msg)
                (telega-chatbuf--topic-match-msg-p msg))))))
...
(defun telega-chatbuf--get-history-internal
    (from-msg-id offset limit &optional callback)
  "Get chatbuf's history messages."
  (declare (indent 3))
  (cond ((plist-get telega-chatbuf--msg-filter :saved-messages-tag)
         (cl-assert (telega-chatbuf-match-p 'saved-messages))
         (telega--searchSavedMessages
             from-msg-id offset
           :tag (plist-get telega-chatbuf--msg-filter
                           :saved-messages-tag)
           :query (plist-get telega-chatbuf--msg-filter :query)
           :limit limit
           :topic telega-chatbuf--topic
           :callback callback))
        ...
        ((telega-topic-match-p telega-chatbuf--topic '(type forum))
         (telega--getForumTopicHistory
             telega-chatbuf--chat telega-chatbuf--topic
             from-msg-id offset limit
           callback))

        ((telega-topic-match-p telega-chatbuf--topic '(type thread))
         (telega--getMessageThreadHistory
             telega-chatbuf--chat (telega-chatbuf--topic-thread-msg)
             from-msg-id offset limit
           callback))

        ((telega-topic-match-p telega-chatbuf--topic '(type sm))
         (telega--getSavedMessagesTopicHistory
             telega-chatbuf--topic from-msg-id offset
           :limit limit
           :callback callback))

        ((telega-topic-match-p telega-chatbuf--topic '(type dm))
         ;; TODO: getDirectMessagesChatTopicHistory
         (error "TODO: getDirectMessagesChatTopicHistory")
         )

        (t
         (telega--getChatHistory
             telega-chatbuf--chat from-msg-id offset
...
argument - total number of loaded messages."
  ...
  (unless telega-chatbuf--history-loading
    ...
    (unless from-msg-id
      ...
      (setq from-msg-id 0               ; Load from last message
            offset -1))

    ;; Asynchronously load chat history
    (cl-assert from-msg-id)
          ...
          (telega-chatbuf--get-history-internal from-msg-id offset limit
            (lambda-with-current-buffer (history)
              ...
              (let* ((messages (plist-get history :messages))
                ...
                (setq telega-chatbuf--history-loading nil)
                (when (eq 0 (plist-get history :next_from_message_id))
                  (telega-chatbuf--older-history-loaded))
                (cond ((and (>= offset 0)
                      ...
                      ((and (< offset 0)
                            (or (zerop (length messages))
                                (and (< offset -1)
                                     (= 1 (length messages))
                                     (eq from-msg-id
                                         (plist-get (seq-first messages) :id)))
                                ))
                       (when (and (zerop from-msg-id)
                                  (= offset -1)
...
(defun telega-chatbuf--load-older-history ()
  ...
  (if (and telega-chatbuf--msg-filter
    ...
    (when (and (not telega-chatbuf--history-loading)
      ...
      (when-let ((msg (plist-get (or telega-chatbuf--topic telega-chatbuf--chat)
                                 :last_message)))
...
(defun telega-chatbuf--load-newer-history ()
  ...
  (if (and telega-chatbuf--msg-filter
    ...
    (when-let ((not-loading-p (not telega-chatbuf--history-loading))
...
Use `\\[universal-argument] to clear formatting from selected region."
  ...
  (if ent-type
      (let (; (telega-inhibit-telega-display-by t)
            ;; HACK: Bind `telega-msg--current' To apply my pallete to
            ;; blockquote/code formatting.  `telega--text-entity-apply'
            ;; uses it for formatting
            (telega-msg--current (telega--MessageSender (telega-user-me))))
        (telega--text-entity-apply
         (list :offset begin
               :length (- end begin)
               :type ent-type)
         ;; nil to apply to current entity to buffer's content
         nil)
        (put-text-property begin end :tl-entity-explicit t))
...
(defun telega-chatbuf--attach-with-caption-p (attach)
  "Return non-nil if attach imc might have a caption."
  (or (memq (telega--tl-type attach)
...
`telega-chat-markup-functions' to be used for input formatting."
  ...
  (let ((markup-function (cdr (assoc markup-name telega-chat-markup-functions)))
        ...
        (link-preview-options telega-chat-send-link-preview-options)
        result)
    (while attaches
      (let* ((text (car attaches))
        ...
        (cond
         ((not attach)
          ...
          ;; NOTE: maybe caption above the media?
          (let* ((next-text (cadr attaches))
                 (next-attach (when next-text
                                (get-text-property 0 'telega-attach next-text))))
            (cond ((and next-attach
                        (or (memq (telega--tl-type next-attach)
                                  '(inputMessageAnimation
                                    inputMessagePaidMedia
                                    inputMessagePhoto
                                    inputMessageVideo))
         ...
         ;; Special attachment to change link preview options
         ...
         ;;    be supplied for the forwarded message only if message
         ;;    is forwarded as copy and original caption is removed
         ;;    (`C-u C-u f' behaviour)
         (t
      ...
      (cond
       ...
       ((and (> (length imcs) 1)
        ...
        (telega--forwardMessages
         ...
         (mapcar (telega--tl-prop :message) send-imcs)
         options
     ...
     (t
      ...
      (let ((imc (car send-imcs)))
        (cl-case (telega--tl-type imc)
           ...
           (let* ((msg (plist-get imc :message))
                  (copy-opts
                   (nconc (list :@type "messageCopyOptions"
                          ...
                          (when-let ((new-cap-above
                                      (plist-get imc :show_caption_above_media)))
                            (list :new_show_caption_above_media new-cap-above))))
          ...
          (telegaChatTheme
           (unless preview-p
             (telega--setChatTheme telega-chatbuf--chat
                                   (plist-get imc :theme))))
...
(defun telega-chatbuf--filter-msg-position-load (message)
  ...
  (when (and telega-chatbuf--msg-filter
    ...
    (let ((tdlib-msg-filter
           (plist-get telega-chatbuf--msg-filter :tdlib-msg-filter)))
      (when (and tdlib-msg-filter
                 (listp tdlib-msg-filter)
                 ;; Do not load position multiple times. However, we
                 ;; probably need some logic to update position at
                 ;; some point
                 (not (plist-get telega-chatbuf--msg-filter :msg-position))
...
(defun telega-chatbuf--topic-reset (&optional no-chat-update-p)
  "Reset `telega-chatbuf--topic' in the chatbuf.
If NO-CHAT-UPDATE-P, do not trigger chatbuf's headers update."
  (setq telega-chatbuf--topic nil)

  (unless no-chat-update-p
    (telega-chatbuf--chat-update "topic")))

(defun telega-chatbuf--clean (&optional keep-predicate)
  "Remove all messages displayed in the chatbuf.
If KEEP-PREDICATE is specified, then keep messages for which
KEEP-PREDICATE return non-nil."
  (if keep-predicate
      (ewoc-filter telega-chatbuf--ewoc keep-predicate)
...
Recenter to the bottom if point is at prompt, otherwise call
`recenter' as-is."
  (interactive "P")
  (if (and (not arg) (<= telega-chatbuf--input-marker (point)))
      (recenter telega-chat-recenter-to-bottom-arg)
...
(defun telega-chatbuf-next-unread (&optional only-loaded-p)
  ...
  (let* ((unread-count (telega-chatbuf--unread-message-count))
         ...
         (unread-node (when last-read-node
                        (ewoc-next telega-chatbuf--ewoc last-read-node))))
    ;; Skip internal and deleted messages
    (while (and unread-node
                (or (telega-msg-internal-p (ewoc-data unread-node))
                    (telega-msg-match-p (ewoc-data unread-node) 'is-deleted)))
      (setq unread-node (ewoc-next telega-chatbuf--ewoc unread-node)))

    (cond (unread-node
           (let ((unread-msg (ewoc-data unread-node)))
             ;; Insert "Unread Messages" above the message
             (telega-chatbuf--manage-unread-messages-bar unread-msg)
             (telega-chatbuf--goto-loaded-msg (plist-get unread-msg :id)
               :observable-how
               (unless (eq (plist-get unread-msg :id)
                           (telega-chatbuf--last-message-id))
                 'top))))

          ((and last-read-node (not (telega-chatbuf--need-newer-history-p)))
           (goto-char (point-max)))

          (only-loaded-p
           ;; History has been fetched, but there is no message to go
           ;; to (probably message has been deleted)
           (telega-chatbuf--goto-approx-msg
            (1+ (telega-chatbuf--last-read-inbox-msg-id)))
           (when-let ((unread-msg (telega-msg-at (point))))
             (telega-chatbuf--manage-unread-messages-bar unread-msg)
             (telega-chatbuf--goto-loaded-msg (plist-get unread-msg :id)
               :observable-how
               (unless (eq (plist-get unread-msg :id)
          ...
          ;; If only last message is unread and it suits filtering, we
          ;; can insert it (with `telega-chatbuf--load-older-history')
          ;; and jump to it.  This will retain current topic and
          ;; filtering
          ((= 1 unread-count)
           (telega-chatbuf--clean)
           (telega-chatbuf--load-older-history)
           (telega-chatbuf--goto-loaded-msg (telega-chatbuf--last-message-id)
             ;; ARGUABLE: jump to prompt if last message it fully observable
             ;; :callback
             ;; (lambda (button)
          ...
          (t
           ...
           (telega-chatbuf--load-history
               (telega-chatbuf--last-read-inbox-msg-id)
               (- (/ telega-chat-history-limit 2)) nil
             (lambda (_ignored)
               (telega-chatbuf-next-unread 'only-loaded)))))))

(defun telega-chatbuf-next-outgoing ()
  ...
  (let* ((reply
          (telega--searchChatMessages telega-chatbuf--chat
              ...
              0
            :sender (telega-user-me)
            :limit 1))
         (next-outgoing-msg
          (car (append (plist-get reply :messages) nil))))
    (unless next-outgoing-msg
      (user-error "telega: No more outgoing messages"))
    (telega-msg-goto next-outgoing-msg 'highlight)
    ))
...
(defun telega-chatbuf-next-unread-reaction ()
  ...
  (telega--searchChatMessages telega-chatbuf--chat
      '(:@type "searchMessagesFilterUnreadReaction") 0 0
    :limit 4
    :callback
    (lambda (reply)
      (let ((reaction-messages (plist-get reply :messages)))
        (if (seq-empty-p reaction-messages)
...
(defun telega-chat-direct-messages-chat (chat)
  "Return chat for the direct messages to the CHAT's channel."
  (let ((supergroup (telega-chat--supergroup-locally chat))
        (telega-full-info-offline-p nil))
    (when (plist-get supergroup :has_direct_messages_group)
      (telega-chat-get
       (plist-get (telega--full-info supergroup) :direct_messages_chat_id)
       'offline))))

(defun telega-chatbuf-goto-direct-messages-chat ()
  "Goto chat linked to current chat buffer channel."
  (interactive)
  (let ((dm-chat (telega-chat-direct-messages-chat telega-chatbuf--chat)))
    (unless dm-chat
      (user-error "telega: %s has no direct messages chat"
                  (telega-chat-title telega-chatbuf--chat)))

    (telega-chat--pop-to-buffer dm-chat)))
...
(defun telega-chatbuf-goto-thread-message ()
  ...
  (interactive)
  (unless (telega-chatbuf--topic-thread-msg)
    (user-error "telega: No thread filtering in the chatbuf"))

  (telega-msg-goto-highlight (telega-chatbuf--topic-thread-msg)))
         ...
         (lambda-with-current-buffer (msg)
           (if (not msg)
             ...
             (telega-chatbuf-inplace-search
              ...
              'forward
              msg)))))
    (cond ((telega-topic-match-p telega-chatbuf--topic '(type sm))
           (telega--getSavedMessagesTopicMessageByDate
               telega-chatbuf--topic req-date callback))
          ((telega-topic-match-p telega-chatbuf--topic '(type dm))
           ;; TODO
           ;; (telega--getDirectMessagesChatTopicMessageByDate
           ;;  telega-chatbuf--chat telega-chatbuf--topic req-date callback)
           )
          (t
           (telega--getChatMessageByDate
               telega-chatbuf--chat req-date callback)))))
...
ahead in case `telega-chat-upload-attaches-ahead' is non-nil."
  ...
  (if (string-match-p "^https?://" filename)
    ...
    (let ((preview (when (and preview-p (> (telega-chars-xheight 1) 1))
                     (telega-create-image filename nil nil
                       :scale 1.0
                       :ascent 'center
                       :height (telega-ch-height 1))))
          (ufile
           (when telega-chat-upload-attaches-ahead
             (telega-file--upload filename
               :file-type file-type
               :priority 16
...
If `\\[universal-argument]' is given, then attach clipboard as document."
  ...
  (if (eq system-type 'darwin)
    ...
    (if-let* ((urls (and (seq-contains-p (gui-get-selection 'CLIPBOARD 'TARGETS) "text/uri-list" #'string=)
...
sticker sets."
  ...
  (if fav-or-recent-p
    ...
    (let* ((tss-window (save-selected-window
           ...
           (sset (telega-stickerset-completing-read
...
(defun telega-chatbuf-attach-checklist (title allow-add-p allow-done-p
                                              &rest tasks)
  "Attach a checklist with a TITLE."
  (interactive
   (let ((title (telega-read-string-with-custom-emojis
                 (concat (telega-i18n "lng_todo_create_title") ": ")))
         (taskidx 1) task-name tasks)
     (while (not (string-empty-p
                  (setq task-name (telega-read-string-with-custom-emojis
                                   (format "Task %d: " taskidx)))))
       (setq tasks (append tasks
                           (list
                            (list :@type "inputChecklistTask"
                                  :id taskidx
                                  :text (telega-string-fmt-text task-name)))))
       (cl-incf taskidx))

     (nconc (list title
                  (y-or-n-p
                   (concat (telega-i18n "lng_todo_create_allow_add") "? "))
                  (y-or-n-p
                   (concat (telega-i18n "lng_todo_create_allow_mark") "? ")))
            tasks)))

  (telega-chatbuf-input-insert
   (list :@type "inputMessageChecklist"
         :checklist (list :@type "inputChecklist"
                          :title (telega-string-fmt-text title)
                          :others_can_add_tasks (if allow-add-p t :false)
                          :others_can_mark_tasks_as_done
                          (if allow-done-p t :false)
...
(defun telega-chatbuf-attach-link-preview-options ()
  "Change `telega-chat-send-link-preview-options' for the following messages."
  (interactive)
  (telega-chatbuf-input-insert
   (list :@type "telegaLinkPreviewOptions"
         ;; TODO: options
         :options nil
         ))
  (when (eobp)
    (telega-momentary-display
     (propertize (telega-i18n "telega_link_preview_options_help")
                 'face 'telega-shadow))))
...
(defun telega-chatbuf-invite-user-via-link (user)
  ...
  (let* ((chatbuf-info (telega-chat--info telega-chatbuf--chat))
    ...
    (plist-put telega-chatbuf--chat :telega-failed-to-add-users
...
(defun telega-chatbuf--switch-in ()
  ...
  (when-let ((newer-freezed (telega-chatbuf--history-state-get :newer-freezed)))
    ...
    (cond ((plist-get newer-freezed :at-msg)
           (telega-msg-goto (plist-get newer-freezed :at-msg)))

          ((plist-get newer-freezed :prompt-p)
...
(defun telega-chatbuf--killed ()
  ...
  ;; Close chat and cancel any active action and actualize chat's draft
  ...
  ;; Keep cursor track *before* removing chat from buffers alist,
  ;; because `telega-root--keep-cursor-at-chat' might use chatbuf to
  ;; get topic from it.
  (when (eq telega-root-keep-cursor 'track)
    (telega-root--keep-cursor-at-chat telega-chatbuf--chat))

  ;; This makes `has-chatbuf' chat temex to return nil before
  ;; updating the chat.
  (setq telega--chat-buffers-alist
        (assq-delete-all telega-chatbuf--chat telega--chat-buffers-alist))

  ;; chatbuf might be left from other telega start, so it will throw
  ;; "Ewoc node not found" error - ignore it
  (ignore-errors
...
Return non-nil if message MSG has been redisplayed."
...
`(nth 1 telega-chat-input-markups)' markup for editing."
  ...
  (unless (telega-msg-match-p msg '(message-property :can_be_edited))
    (error (telega-i18n "lng_edit_error")))
...
(defun telega-chatbuf-attach-fwd-msg (msg &optional send-copy-p rm-cap-p)
  ...
  (when (and send-copy-p rm-cap-p (eobp))
    (telega-momentary-display
     (propertize (telega-i18n "lng_photo_caption")
                 'face 'telega-shadow))))

(defvar telega-msg-forward--last-chat nil
  "Last chat messages has been forwarded to.")

(defun telega-msg-forward-dwim (messages &optional
                                         remove-sender-p remove-caption-p
                                         chat)
  "Forward messages in a DWIM manner.
...
CHAT for non-interactive use only."
  (interactive
   (let* ((raw-messages
          ...
          (messages
           (progn
             ;; NOTE: ExcludeEvery
             (unless (seq-every-p (telega-match-gen-predicate 'msg
  ...
  (unless chat
    (setq chat (telega-completing-read-chat
                (concat (telega-symbol 'forward) "Forward"
                        (when remove-sender-p " Copy")
                        (when remove-caption-p " NewCap")
  ...
  (setq telega-msg-forward--last-chat chat)
  ...
  (with-current-buffer (telega-chat--pop-to-buffer chat)
    (goto-char (point-max))
    (dolist (msg messages)
      (telega-chatbuf-attach-fwd-msg msg remove-sender-p remove-caption-p))))

(defun telega-msg-forward-dwim-to-many (messages chats &optional remove-sender-p
                                                 remove-caption-p)
  "Forward messages in DWIM manner to multiple CHATS."
  (interactive
   (let ((messages
          (cl-sort (or telega-chatbuf--marked-messages
                   ...
                   #'< :key (telega--tl-prop :id))))
     (unless messages
       (user-error "No messages to forward"))
     (list messages (telega-completing-read-chat-list "Forward to Chats"))))

  (cl-assert messages)
  (when (y-or-n-p (concat (telega-i18n "lng_forward_many_title"
                            :count (length messages))
                          (format " to %d chats? " (length chats))))
    (let ((from-chat (telega-msg-chat (car messages))))
      ...
      (dolist (chat chats)
        (telega--forwardMessages chat from-chat messages nil
                                 remove-sender-p remove-caption-p)))

    (message "telega: %s" (telega-i18n (if (> (length messages) 1)
...
(defun telega-msg-delete-dwim (revoke)
  "Deletes messages in a DWIM manner.
  ...
  (if (and (not (telega-msg-at-down-mouse-3))
    ...
    (when-let ((msg (telega-msg-for-interactive)))
      (if (telega-msg-match-p msg 'is-deleted)
        ...
        (when (y-or-n-p (telega-i18n
                            (cond ((telega-chatbuf-match-p 'saved-messages)
...
(defun telega-msg-report-dwim (_msg)
...
If there is no next link in the current message, move point to the
next message.
Use `C-h .' to show URL at point after moving point to a link."
  (interactive "p")

  (or
   ;; Try next link inside current button
   (when-let* ((button (button-at (point)))
               ...
               (pmobj
                (save-excursion
                  (let (ret)
                    (while (/= ni 0)
                      (setq ret (funcall (if (> ni 0)
                                             #'text-property-search-forward
                                           #'text-property-search-backward)
                                         :telega-link nil
                                         (lambda (_ignored1 link-val)
                                           link-val)
                                         'not-current))
                      (setq ni (cond ((null ret) 0)
                                     ((> ni 0) (1- ni))
                                     ((< ni 0) (1+ ni))
                                     (t (cl-assert nil)))))
                    ret)))
               (link-pos (prop-match-beginning pmobj)))
     (when (<= start link-pos end)
       (goto-char link-pos)
       ;; NOTE: Use `C-h .' to get show url at point
       ))
...
If there is no previous link in the current message, move point to the
previous message.
Use `C-h .' to show URL at point after moving point to a link."
  (interactive "p")
...
(cl-defun telega-chatbuf--goto-loaded-msg (msg-id &key highlight-p
                                                  observable-how callback)
  "In chatbuf goto message denoted by MSG-ID.
Goto message only if it is already displayed in the chatbuf.
If HIGHLIGHT-P is non-nil, then momentary highlight the message.
OBSERVABLE-HOW specifies `how' argument for `telega-button--make-observable'.
Return non-nil on success."
  (declare (indent 1))
  (let ((msg-button (button-at (point)))
        (ret nil))
    ;; Possibly point is already on the corresponding message, in this
    ;; case we don't need to move it
    (unless (and msg-button
                 (eq (button-type msg-button) 'telega-msg)
                 (eq msg-id (plist-get (button-get msg-button :value) :id)))
      (setq msg-button nil)
      (when-let ((node (telega-chatbuf--node-by-msg-id msg-id)))
        (ewoc-goto-node telega-chatbuf--ewoc node)
        (setq msg-button (button-at (point))
              ;; NOTE: node could exists, where button is not!  This
              ;; is true for ignored messages if
              ;; `telega-ignored-messages-visible' is nil
              ret t)))

    (when msg-button
      (setq ret t)
      (telega-button--make-observable msg-button observable-how)
      (telega-button--help-echo msg-button)
      (when highlight-p
      ...
      (when telega-msg-goto-content
        (telega-chatbuf--goto-msg-content))
      (when callback
        (funcall callback msg-button)))
    ret))

(defun telega-chatbuf--goto-msg (msg-id &optional highlight-p callback)
  "In CHAT goto message denoted by MSG-ID.
If HIGHLIGHT-P is non-nil then highlight with fading background color.
...
CALLBACK is called after point is moved to the message with MSG-ID."
  ...
  ;;    messages ring
  ;; 2. If message is shown in the chatbuf, jump to it
  ;; 3. Otherwise, fetch history containing message and jump to it
  (when (ring-p telega-chatbuf--messages-pop-ring)
    (when-let (msg-at-point (telega-msg-at (point) #'telega-msg-from-history-p))
      (unless (or (eq msg-id (plist-get msg-at-point :id))
  ...
  (if (or (zerop msg-id)
          (telega-chatbuf--goto-loaded-msg msg-id :highlight-p highlight-p))
      (when callback
        (funcall callback))

    ;; 1. If message is available in the cache, then show it
    ;; 2. Load history around the message
    (let ((cached-msg (telega-msg-get telega-chatbuf--chat msg-id)))
      (telega-chatbuf--clean)

      (when cached-msg
        (unless (telega-chatbuf--filter-match-msg-p cached-msg)
          (telega-chatbuf--filter-reset))
        (unless (telega-chatbuf--topic-match-msg-p cached-msg)
          (telega-chatbuf--topic-reset))
        (telega-chatbuf--insert-messages (list cached-msg) 'append-new)
        (telega-chatbuf--goto-loaded-msg msg-id :highlight-p highlight-p)
        (when callback
          (funcall callback)))
      ...
      (telega-chatbuf--load-history
        ...
        (unless cached-msg
          (lambda (_ignored)
            (or (telega-chatbuf--goto-loaded-msg msg-id
...
(defun telega-chatbuf--goto-msg-content (&optional text-position)
  "Goto the beginning of the current message's content.
If TEXT-POSITION is specified, move point forward to this offset
ensuring point keep being inside the message."
  (when-let* ((msg-button (button-at (point)))
              (start (button-start msg-button))
              (limit (button-end msg-button))
              (content-prop (if text-position :message-text :message-content))
              (mc-pos (+ (if (get-text-property start content-prop)
                             start
                           (next-single-property-change
                            start content-prop nil limit))
                         (or text-position 0))))
    (when (< mc-pos limit)
      (goto-char mc-pos))))

(defun telega-chat--goto-msg (chat msg-id &optional highlight-p callback)
  "Pop chatbuf for the CHAT and goto message denoted by MSG-ID."
  (declare (indent 3))

  (with-current-buffer (telega-chat--pop-to-buffer chat :no-history)
    (telega-chatbuf--goto-msg msg-id highlight-p callback)))
...
(defun telega-chat--goto-thread (chat thread-id &optional reply-msg-id)
  ...
  (let* ((telega-server-call-timeout 3.0)
    ...
    ;; ;; NOTE: `thread-msg' itself is not part of the thread, only replies are.
    ;; ;; So, if `reply-msg-id' is actually `thread-msg' we need to load
    ;; ;; initial history as usual
    ;; (when (eq reply-msg-id (plist-get thread-msg :id))
    ;;   (setq reply-msg-id nil))

    (with-current-buffer (telega-chat--pop-to-buffer thread-chat :no-history)
      (telega-chatbuf-filter-by-topic
       thread-info
       reply-msg-id))))
...
(defconst telega-chat--message-filters
  `(("favorite"
    ...
    ("topic"
     (or saved-messages is-forum is-direct-messages-group)
     telega-chatbuf-filter-by-topic)
    ("tag"
     saved-messages telega-chatbuf-filter-by-saved-messages-tag)
...
(defconst telega-chat--message-filter-temex-alist
  '((searchMessagesFilterEmpty .     (return nil))
    ...
    (searchMessagesFilterPinned .    is-pinned))
...
(defun telega-chatbuf--topic-match-msg-p (msg)
  "Return non-nil if MSG matches currently active chatbuf's topic."
  (if telega-chatbuf--topic
      (equal (telega--MessageTopic telega-chatbuf--topic)
             (plist-get msg :topic_id))
...
(defun telega-chatbuf-filter (msg-filter)
  ...
  (cl-assert (and msg-filter (or (plist-get msg-filter :tdlib-msg-filter)
                                 (plist-get msg-filter :saved-messages-tag))))
  (if (commandp (plist-get msg-filter :tdlib-msg-filter) 'for-interactive)
    ...
    (let* ((msg-at-point (telega-msg-at (point) #'telega-msg-from-history-p))
      ...
      (telega-chatbuf--load-history
          (when keep-msg-p (plist-get msg-at-point :id))
          (if keep-msg-p (- (/ telega-chat-history-limit 2)) -2)
          telega-chat-history-limit
        (lambda (total-messages)
          (when telega-chatbuf--msg-filter
            (plist-put telega-chatbuf--msg-filter :total-count total-messages)
            (telega-chatbuf--chat-update "msg-filter"))
          (if (and msg-at-point (not keep-msg-p))
              (telega-chatbuf--goto-loaded-msg (plist-get msg-at-point :id))
            (recenter telega-chat-recenter-to-bottom-arg))))
    ...
    (when-let ((query (telega-tl-str msg-filter :query)))
...
If START-MSG-ID is specified, jump to the this message in the topic."
  (interactive
   (if (telega-chat-match-p telega-chatbuf--chat
         (nth 1 (assoc "topic" telega-chat--message-filters)))
       (list (telega-completing-read-topic telega-chatbuf--chat) nil)
   (user-error "Can't read topic for non-forum chat")))

  (when (or start-msg-id (not (eq topic telega-chatbuf--topic)))
    (setq telega-chatbuf--topic topic)
    (telega-chatbuf--chat-update "topic")

    ;; Try to keep message at point after topic filtering is applied
    (let* ((msg-at-point
            (telega-msg-at (point) #'telega-msg-from-history-p))
           (keep-msg-p
            (and msg-at-point
                 (or (not start-msg-id)
                     (eq (plist-get msg-at-point :id) start-msg-id))
                 ;; NOTE: starting message will be inserted in the
                 ;; `telega-chatbuf--older-history-loaded'
                 ;; (not (telega-msg-id=
                 ;;       msg-at-point (telega-chatbuf--topic-thread-msg)))
                 (telega-chatbuf--topic-match-msg-p msg-at-point))))
      (cond (keep-msg-p
             (telega-chatbuf--clean (apply-partially #'eq msg-at-point))
             (telega-chatbuf--manage-discussion-bar
              (ewoc-nth telega-chatbuf--ewoc 0))
             ;; Load history around the message
             (telega-chatbuf--load-history
...
(defun telega-chatbuf-filter-by-saved-messages-tag (tag &optional query)
  ...
  (interactive (list (telega-completing-read-saved-messages-tag "Tag: ")
                     (when current-prefix-arg
                       (read-string "Search Query: "))))
  (unless (telega-chatbuf-match-p 'saved-messages)
    (user-error "Can filter by tag only in Saved Messages"))

  (let ((reaction-type (plist-get tag :tag)))
    (telega-chatbuf-filter
     (list :title (lambda ()
                    (telega-ins--as-string
                     (telega-ins "Tag ")
                     (telega-ins--saved-messages-tag
                      (telega-saved-messages-find-tag reaction-type))))
           :saved-messages-tag tag
           :total-count (plist-get tag :count)
           :query query
           :msg-temex
           ;; Match messages having `tag' set
           (list 'has-chosen-reaction reaction-type)))))
...
If `\\[universal-argument]' is given, then cancel topic filtering as well."
  ...
  (when (or telega-chatbuf--msg-filter
            (and topic-cancel-p telega-chatbuf--topic))
    (when telega-chatbuf--msg-filter
      (telega-chatbuf--filter-reset))
    (when (and topic-cancel-p telega-chatbuf--topic)
      (telega-chatbuf--topic-reset))

    ;; NOTE: if point is at some message, then keep this
    ;; message visible, otherwise load initial history
    (let (msg)
      (cond ((setq msg (telega-msg-at (point) #'telega-msg-from-history-p))
             (telega-chatbuf--clean (apply-partially #'eq msg))
             (when (telega-chatbuf--need-newer-history-p)
               (telega-chatbuf--load-newer-history)))

            ((setq msg (when topic-cancel-p
                         (telega-chatbuf--topic-thread-msg)))
             (telega-chatbuf--clean)
...
(defvar telega-chatbuf-inplace-search-history nil
  "Inplce search history.")
...
(defun telega-search-query-as-regexp (query)
  ...
  (when-let ((tokens (split-string query telega-history-search-separators t)))
    (propertize (regexp-opt tokens)
                ...
                ;; Limit to 15 chars
                (telega-ins--as-string
                 (telega-ins--with-attrs '(:max 15)
                   (telega-ins (mapconcat #'identity tokens " ")))))))
...
non-interactive use cases only."
  ...
  (if (commandp (plist-get isearch-filter :tdlib-msg-filter) 'for-interactive)
    ...
    (let* ((tl-msg-filter
           ...
           (query-regexp (when query (telega-search-query-as-regexp query)))
              ...
              (let* ((next-msg
                     ...
                     (found-p (and next-msg
                                   (not (eq (plist-get next-msg :id)
                                            from-msg-id)))))
                (when (or (not found-p)
                          (and telega-highlight-text-mode
                               (not (equal query-regexp
                                           telega-highlight-text-regexp))))
                  (telega-highlight-text-mode -1)
                  (telega-chatbuf--chat-update "highlight-text"))
                ...
                (if (not found-p)
                  ...
                  (when query-regexp
                    (telega-highlight-text query-regexp)
                    (telega-chatbuf--chat-update "highlight-text"))
      ...
      (cond ((or (plist-get telega-chatbuf--msg-filter :saved-messages-tag)
             ...
             (when by-sender
               (user-error
                "telega: Can't search by sender in Saved Messages tags"))
             ;; Inplace search inside Saved Message tag filter
             ;; Use `searchSavedMessages' in this case
             (telega--searchSavedMessages
                 (or from-msg-id 0) (if forward-p -2 0)
               :query query
               :limit 3
               :tag (plist-get telega-chatbuf--msg-filter :saved-messages-tag)
               :topic telega-chatbuf--topic
               :callback callback))

            ((and (plist-get isearch-filter :tdlib-msg-filter)
                  (plist-get telega-chatbuf--msg-filter :tdlib-msg-filter)
                  (not (equal (plist-get isearch-filter :tdlib-msg-filter)
                              (plist-get telega-chatbuf--msg-filter
                                         :tdlib-msg-filter))))
             (user-error
              "telega: Can't search inplace while message filter is applied"))

            (t
             (telega--searchChatMessages telega-chatbuf--chat
                 (or tl-msg-filter '(:@type "searchMessagesFilterEmpty"))
                 (or from-msg-id 0) (if forward-p -2 0)
               :query query :limit 3 :sender by-sender
               :topic telega-chatbuf--topic
               :callback callback))))))
...
bindings."
  (interactive
   (list (read-string "Telega-search (backward): " nil
                      'telega-chatbuf-inplace-search-history)
         current-prefix-arg))
  (let ((by-sender (when by-sender-p
                     (telega-completing-read-chat-member
                      "Sent by: " telega-chatbuf--chat
...
containing QUERY sent by specified sender."
  (interactive
   (list (read-string "Telega-search (forward): " nil
                      'telega-chatbuf-inplace-search-history)
         current-prefix-arg))
...
(defun telega-chat-theme--choosen-action (theme)
  ...
  (let ((chat telega--chat))
    ...
    (with-telega-chatbuf chat
      (telega-chatbuf-input-insert
       (nconc (list :@type "telegaChatTheme")
              (when theme
                (list :theme (list :@type "inputChatThemeEmoji"
...
(defun telega-describe-chat-themes (&optional themes for-chat)
  ...
  (let ((themes (or themes telega--chat-emoji-themes))
    ...
    (with-telega-help-win "*Telegram Chat Themes*"
      ...
      (when (plist-get for-chat :theme)
        (telega-ins " ")
        (telega-ins--ui-button (telega-i18n "lng_chat_theme_reset")
          :value nil
...
(defun telega-chatbuf-attach-chat-theme ()
  ...
  (interactive)
  (telega-describe-chat-themes
   telega--chat-emoji-themes telega-chatbuf--chat))
...
(defun telega-chat-buffer-auto-fill (&optional win)
  ;; In analogue to `telega-root-buffer-auto-fill'.
  (interactive (list (get-buffer-window)))

  (when (and (or (null win) (eq (selected-window) win))
             ;; NOTE: For some reason if `global-display-line-numbers-mode' is
             ;; enabled, then `display-line-numbers-mode-hook' might be called
             ;; with current window displaying non chat buffer
             (buffer-local-value 'telega-chatbuf--chat (window-buffer win)))
    (let* ((win-margins (window-margins win))
      ...
      (when (and new-fill-column
                 ...
                 (or (not (eq new-fill-column fill-column))
                     (not (eq new-chat-fill-column telega-chat-fill-column))))
        (let ((progress (make-progress-reporter
                         (format "telega: chatbuf auto fill %d -> %d ..."
                                 telega-chat-fill-column new-chat-fill-column))))
          (with-current-buffer (window-buffer win)
            (setq-local telega-chat-fill-column new-chat-fill-column
...
(defvar telega-chat-auto-fill-mode-lighter
  (concat " " (telega-symbol 'mode) "Auto-Fill")
  "Lighter for the `telega-chat-auto-fill-mode'.")

(define-minor-mode telega-chat-auto-fill-mode
  "Toggle chatbuf autofill mode."
  :lighter telega-chat-auto-fill-mode-lighter
  (if telega-chat-auto-fill-mode
      (progn
