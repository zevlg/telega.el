...
=telega= depends on the =visual-fill-column= and =transient= packages.
This dependency automatically installs if you install =telega= from
MELPA or GNU Guix.  Otherwise will you need to install these packages
by hand.
...
Pre-built =telega-server= and =TDLib= binaries for Windows x86-64 are
available from [[https://github.com/zHaOdANiuu/telega-rc][telega-rc]].

Thanks to [[https://t.me/zdn_y][=@zdn_y=]]
...
ask you to authorize via QR-code using official Telegram application.
...
** Using proxy to access Telegram

#+ELLIT-INCLUDE: ../telega-info.el :label proxies
...
**NOTE**: Voice/video calls are not supported by =telega=.
...
- {{{user-option(telega-avatar-workaround-gaps-for, 2)}}}
...
{{{kbd(C-j)}}} Emacs command.  To preview resulting message before
sending it, press
{{{where-is(telega-chatbuf-input-preview,telega-chat-mode-map)}}}.
...
forward and press
{{{where-is(telega-transient-msg-forward,telega-msg-button-map)}}}.
To forward multiple messages at once, mark messages with the
...
{{{where-is(telega-transient-msg-delete,telega-msg-button-map)}}}.
...
** Completing input in a chatbuf

It is possible to complete chatbuf's input with
{{{where-is(telega-chatbuf-complete-or-next-link,telega-chat-mode-map)}}}.

You can use standard Emacs ~capf~ (completion at point) or advanced
[[#telega-company][telega-company]].

To enable ~capf~ completions use:
#+begin_src elisp
...
**NOTE**: Voice calls and Video chats are not supported by =telega=.
...
Or you can press {{{where-is(telega-transient-msg-save,telega-msg-button-map)}}}
...
{{{where-is(telega-transient-msg-save,telega-msg-button-map)}}} on the
corresponding animation message.

* Stories
#+ELLIT-INCLUDE: ../telega-story.el :label commentary
* Multiple accounts
...
#+ELLIT-INCLUDE: ../telega-transient.el :label minor-modes
...
#+ELLIT-INCLUDE: ../contrib/telega-company.el
...
**A**: If your Emacs does not have builtin WEBP format
