...
(defconst telega--imagemagick-enabled-p (image-type-available-p 'imagemagick))

(defun telega-msg-sticker (msg)
  "Format sticker message MSG."
  ...
  (let* ((content (plist-get msg :content))
         ...
         (preview-path (and telega--imagemagick-enabled-p
                            (or (plist-get content :preview)
                                (telega-file--get-path-or-start-download
                                 sticker-file
                                 (plist-get msg :chat_id)
                                 (plist-get msg :id)))))
         (image (when preview-path (create-image preview-path 'imagemagick nil :width 128)))
