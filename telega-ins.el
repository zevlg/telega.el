...
(defun telega-ins--box-button-delimiter (bb-style prop-delim)
  "Insert box button vertical delimiter."
;  (telega-ins--with-props '(cursor-intangible t rear-nonsticky t)
    (telega-ins (telega-box-button--style-get bb-style prop-delim)))
;)
...
If WITH-YEARS-OLD-P is specified, insert years old as well."
  (let ((bd-day (telega-tl-get0 birthdate :day))
        (bd-month (telega-tl-get0 birthdate :month))
        (bd-year (telega-tl-get0 birthdate :year)))
    ;; NOTE: Birthday year might not be specified
    (if (and with-years-old-p (not (zerop bd-year)))
      ...
      (let* ((bd-decoded (list 0 0 0 bd-day bd-month bd-year))
        ...
        (telega-ins--date bd-timestamp (if (zerop bd-year)
                                           "%d %B"
                                         'date-long))))))

(defun telega-ins--date-relative (timestamp &optional force-relative-p)
  "Insert relative date for the timestamp.
FORCE-RELATIVE-P forces relative formatting, used to format
`dateTimeFormattingTypeRelative' timestamps."
    ...
    (cond ((and (> timestamp yesterday00)
          ...
          ((and (> timestamp today00)
           ...
           (telega-ins-i18n "lng_mediaview_today"
             :time formatted-time))
          ((and force-relative-p
                (< timestamp today00))
           (telega-ins-i18n "lng_date_relative_days_ago"
             :count (/ (- today00 timestamp)
                       (* 24 3600))))
          (t
...
If MSG-SENDER is a user (or private/secret chat), then WITH-TITLE can
be one of `first-name', `last-name', `full-name' or `username'.  By
default `full-name' is used as user title.
WITH-BADGES-P is ignored if WITH-TITLE is specified."
  (declare (indent 1))
  (cl-assert (or (stringp with-title)
                 (memq with-title
                       '(nil first-name last-name full-name username))))
  (let* ((chat-p (telega-chat-p msg-sender))
         (title (cond ((stringp with-title) with-title)
                      (chat-p
                       (telega-chat-title msg-sender with-title
                                          (not with-badges-p)))
                      (t
                       (cl-assert (telega-user-p msg-sender))
                       (telega-user-title msg-sender
                                          (or with-title 'full-name)
                                          (not with-badges-p)))))
...
(defun telega-ins--chat-member-tag (member &optional only-tag-p)
  "Insert MEMBER's tag or owner/admin badge."
  (when-let* ((status (plist-get member :status))
              (member-tag
               (or (telega-tl-str member :tag)
    ...
    (telega-ins--with-style
        (telega-box-button-style
            (cl-case (telega--tl-type status )
              (chatMemberStatusCreator       'owner-sender-tag)
              (chatMemberStatusAdministrator 'admin-sender-tag)
              (t                             'sender-tag)))
      (telega-ins member-tag))
    t))
...
If SHOW-PHONE-P is non-nil, then show USER's phone number."
  (let ((avatar (telega-msg-sender-avatar-image user))
    ...
    (telega-ins-prefix " "
      (telega-ins--chat-member-tag member))
    ...
    (when-let ((join-date (telega-tl-get0 member :joined_chat_date)))
      (unless (zerop join-date)
        (telega-ins ", " (telega-i18n "lng_group_invite_joined_status"
                           :date ""))
        (telega-ins--date join-date)))

    (when-let* ((inviter-id (telega-tl-get0 member :inviter_user_id))
...
(defun telega-ins--file-progress (msg file)
  ...
  (cond ((telega-file--uploading-p file)
         (let* ((progress (telega-file--uploading-progress file))
           ...
           (telega-ins--ui-button (telega-i18n "lng_context_cancel_upload")
        ...
        ((telega-file--downloading-p file)
         (let* ((progress (telega-file--downloading-progress file))
           ...
           (when (< progress-100 10)
             (telega-ins nbsp-char))
           (telega-ins-fmt "%d%%]%c" progress-100 nbsp-char)
           (telega-ins--ui-button (telega-i18n "lng_context_cancel_download")
             'action (lambda (_ignored)
    ...
    (let ((sending-state (plist-get (plist-get msg :sending_state) :@type))
      ...
      (telega-ins
       (cond ((plist-get msg :scheduling_state)
             ...
             ((>= (telega-tl-get0 chat :last_read_outbox_message_id)
                  (plist-get msg :id))
  ...
  (let* ((hr (telega-photo--highres photo))
    ...
    (when (or show-details show-progress)
      ...
      (when show-progress
        (telega-file--download hr-file
          :priority 20
          :update-callback
          ;; NOTE: MSG can be a sponsoredMessage
          (when (telega-msg-p msg)
            (lambda (_fileignored)
              (telega-msg-redisplay msg)))))
    ...
    (let ((msg-content (plist-get msg :content)))
      (cond ((and (plist-get msg-content :is_secret)
             ...
             (let ((ttl-in (telega-tl-get0 msg :self_destruct_in)))
               (unless (zerop ttl-in)
                 (telega-ins--with-face 'telega-shadow
            ...
            (t
             ...
             (when (plist-get msg-content :has_spoiler)
               ...
               (telega-ins "\n")
               (telega-ins--ui-button (telega-i18n "lng_context_spoiler_effect")
                 :action #'telega-msg-media-spoiler-toggle)
               (telega-ins " "))
             )))
    t))
...
If MUSIC-SYMBOL is specified, use it instead of play/pause."
  ...
  (let (ret)
    (unless (eq how 'thumbnail)
      (let* ((dur (telega-tl-get0 audio :duration))
    ...
    (unless (eq how 'thumbnail)
      ...
      (telega-ins
       ...
       (let ((dur (telega-tl-get0 video :duration)))
         (unless (zerop dur)
           (concat " " (telega-duration-human-readable dur))))
       ")")
...
(defun telega-ins--ffplay-controls (msg &optional no-2x-button)
  "Insert controls for audio/voice/video message.
If NO-2X-BUTTON is specified, then do not display \"2x\" button."
  (when-let ((proc (plist-get msg :telega-ffplay-proc)))
    (when (telega-ffplay-playing-p proc)
      (telega-ins--ui-button (telega-symbol 'rewind-backward)
        'action (lambda (_button)
                  (telega-msg--vvnote-rewind msg -10)))
      (telega-ins " ")
      (telega-ins--ui-button (telega-symbol 'rewind-forward)
        'action (lambda (_button)
                  (telega-msg--vvnote-rewind msg 10)))
      (telega-ins " ")
      ...
      (unless no-2x-button
        (let* ((label2x (format "%g×" telega-vvnote--play-speed))
          ...
          (telega-ins--ui-button label2x
            :value msg
            :action #'telega-msg--vvnote-play-speed-toggle))
        (telega-ins " ")))

    (cond ((telega-ffplay-playing-p proc)
           (telega-ins--ui-button (telega-i18n "lng_mac_menu_player_pause")
             :value msg
             :action #'telega-msg-open-content)
           (telega-ins " "))
          ((telega-ffplay-paused-p proc)
           (telega-ins--ui-button (telega-i18n "lng_mac_menu_player_resume")
             :value msg
             :action #'telega-msg-open-content)
           (telega-ins " ")))

    (telega-ins--ui-button (telega-i18n "lng_mac_menu_player_stop")
      :value msg
...
(defun telega--can-speech-recognize-p (&optional duration)
  ...
  (or (telega-user-match-p (telega-user-me) 'is-premium)
      (and telega--speech-recognition-trial
           (> (telega-tl-get0 telega--speech-recognition-trial :left_count) 0)
           (or (null duration)
    ...
    (pcase lp-tl-type
      ...
      ('linkPreviewTypeBackground
       (when-let ((doc (plist-get lp-type :document)))
         ...
         (telega-ins "\n")))
      ((or 'linkPreviewTypeChannelBoost
      ...
      ('linkPreviewTypeDocument
       (when-let ((doc (plist-get lp-type :document)))
         (when (telega-ins--document msg doc 'thumbnail)
           (telega-ins "\n"))
         (telega-ins--with-face 'telega-shadow
           (telega-ins--document msg doc 'metainfo))
         (telega-ins "\n")))
      ('linkPreviewTypeEmbeddedVideoPlayer
       (let ((video (plist-get lp-type :video))
             (thumb (plist-get lp-type :thumbnail)))
         (cond (video
                (when (or (telega-ins--video msg video 'thumbnail)
                          (and thumb telega-link-preview-preview-size-limits
                               (telega-ins--photo
                                thumb msg
                ...
                (telega-ins "\n"))
               ((and thumb telega-link-preview-preview-size-limits)
                (telega-ins--photo thumb msg
                                   telega-link-preview-preview-size-limits)
                (telega-ins "\n")))))
      ((or 'linkPreviewTypeEmbeddedAnimationPlayer
      ...
      ('linkPreviewTypeVideo
       (when-let ((video (plist-get lp-type :video)))
         ...
         (telega-ins--with-face 'telega-shadow
           (telega-ins--video msg video 'metainfo)
           (let ((start-timestamp (telega-tl-get0 lp-type :start_timestamp)))
             (unless (zerop start-timestamp)
      ...
      (_
       (error "Unsupported %S" lp-tl-type)))
    t))
...
(defun telega-ins--link-preview-button (_msg link-preview)
  ...
  (let* ((lp-type (plist-get link-preview :type))
         (iv-button-p
          (not (zerop (telega-tl-get0 link-preview :instant_view_version))))
         (button-label
          (if iv-button-p
            ...
            (cl-case (telega--tl-type lp-type)
              ...
              (linkPreviewTypeGiftAuction
               (telega-i18n "lng_auction_preview_view_results"))
              (linkPreviewTypeVideoChat
    ...
    (when button-label
      ;; Adjust button-label
      (setq button-label (upcase button-label))
      (when iv-button-p
        (setq button-label (concat (telega-symbol 'lightning)
                                   button-label)))

      (telega-ins--box-button2 button-label
...
(defun telega-ins--contact-msg (msg)
  ...
  (let* ((content (plist-get msg :content))
         ...
         (user-id (telega-tl-get0 contact :user_id))
        ...
        (giveawayPrizeStars
         (telega-ins-i18n "lng_prizes_credits_about"
           ...
           :amount (telega-i18n "lng_prize_credits_amount"
                       :count (telega-tl-get0 prize :star_count)))))
      (telega-ins "\n"))
  ...
  (let* ((content (plist-get msg :content))
         ...
         (cost (propertize
                (telega-i18n "lng_action_gift_for_stars"
                  :count (telega-tl-get0 gift :star_count))
                'face 'bold)))
    (telega-ins--with-face 'telega-shadow
    ...
    (unless header-only-p
      ...
      (or (telega-ins--fmt-text (plist-get content :text))
          (cond ((telega-msg-match-p msg 'is-outgoing)
                 (telega-ins-i18n "lng_action_gift_sent_text"
                   :user (telega-msg-sender-title--special
                             (telega-chat-user (telega-msg-chat msg)))
                   :count (telega-tl-get0 content :sell_star_count)))
      ...
      (unless (telega-msg-match-p msg 'is-outgoing)
        (telega-ins--ui-button (telega-i18n "lng_sticker_premium_view")
    ...
    (telega-ins--box-button
        (telega-i18n "lng_paid_price"
          :price (telega-ins--as-string
                  ...
                  (telega-ins-fmt "%d" (telega-tl-get0 content :star_count))))
      )
    (telega-ins "\n")
...
(defun telega-ins--checklist-msg (msg)
  ...
  (let* ((content (plist-get msg :content))
    ...
    (when (plist-get checklist :can_add_tasks)
      (telega-ins " ")
      (telega-ins--ui-button (telega-i18n "lng_todo_add_title")
...
(defun telega-ins--currency-amount (currency total-amount)
  "Inserter for the currency amount."
  (if (equal currency "XTR")
      (progn
        (telega-ins (telega-symbol 'telegram-star))
        (telega-ins--with-face 'bold
          (telega-ins-fmt "%d" total-amount)))

    (telega-ins--with-face 'bold
      (format "%.2f" (/ total-amount 100.0)))
    (telega-ins (or (alist-get currency telega-currency-symbols-alist)
                    currency))))
    ...
    (cl-case (telega--tl-type content)
      ...
      (messageProximityAlertTriggered
       (let ((traveler (telega-msg-sender (plist-get content :traveler_id)))
             ...
             (distance (telega-tl-get0 content :distance)))
      ...
      (messageChatSetMessageAutoDeleteTime
       (let* ((sender-me-p (telega-me-p sender))
              (auto-del-time (telega-tl-get0 content :message_auto_delete_time)))
      ...
      (messagePaymentSuccessful
       (telega-ins-i18n (if (plist-get content :is_recurring)
         ...
         :amount (telega-ins--as-string
                  (telega-ins--currency-amount
                   (telega-tl-str content :currency)
      ...
      (messagePaymentRefunded
       (telega-ins-i18n "lng_action_payment_refunded"
         :amount (telega-ins--as-string
                  (telega-ins--currency-amount
                   (telega-tl-str content :currency)
                   (plist-get content :total_amount)))
         :peer (telega-msg-sender-title--special
                   (telega-msg-sender (plist-get content :owner_id)))))
      (messageForumTopicCreated
      ...
      (messageGiveawayCompleted
       (telega-ins-i18n "lng_action_giveaway_results"
         :count (telega-tl-get0 content :winner_count)))
      (messageGiftedPremium
       (let* ((gifter-user-id (telega-tl-get0 content :gifter_user_id))
              (_gifter (unless (zerop gifter-user-id)
      ...
      (messageChatBoost
       (telega-ins-i18n "lng_action_boost_apply"
         :count (telega-tl-get0 content :boost_count)
         :from sender-name))
      (messageUsersShared
       (let ((shared-users
              (mapcar #'telega-user-get
      ...
      (messageDirectMessagePriceChanged
       (let ((enabled-p (plist-get content :is_enabled))
             (star-count (telega-tl-get0 content :paid_message_star_count)))
         (cond ((and enabled-p (not (zerop star-count)))
      ...
      (messageChatHasProtectedContentToggled
       (let ((protect-p (plist-get content :new_has_protected_content))
             (sender-me-p (telega-me-p sender)))
         (if sender-me-p
             (telega-ins-i18n (if protect-p
                                  "lng_action_no_forwards_you_disabled"
                                "lng_action_no_forwards_you_enabled"))
           (telega-ins-i18n (if protect-p
                                "lng_action_no_forwards_disabled"
                              "lng_action_no_forwards_enabled")
             :from sender-name))))
      (messageSuggestBirthdate
...
(defun telega-ins--content (msg)
  ...
  (let* ((telega-msg--current msg)
         ...
         (translated-text (telega-tl-str translated :text))
         (summary (plist-get msg :telega-summary))
         (summary-text (telega-tl-str summary :text)))

    (when (and caption caption-above-p)
      (when (cond ((and translated-text
                        (not (plist-get translated :show-original-p)))
                   (telega-ins translated-text))
                  (summary-text
                   (telega-ins summary-text))
                  (t
    ...
    ;; Translation
    (when (and translated-text
               (plist-get translated :show-original-p))
      (telega-ins--with-face 'telega-shadow
        (telega-ins "\n")
        (telega-ins "--- "
                    (telega-i18n "lng_translate_bar_to"
...
ADDITIONAL-ACTION is called with two args kbd-button and message."
  ...
  (let* ((kbdb-text (or label (telega-ins--keyboard-button-label
         ...
         (bb-style (or forced-style
                       (cl-ecase (telega--tl-type kbdb-style)
 ...
 ;; Layout: list of (width . spacing) conses for each button
  (when (eq (telega--tl-type reply-markup)
    ...
    (if (plist-get reply-markup :resize_keyboard)
        (- telega-chat-fill-column (telega-current-column))

      (let ((rows (plist-get reply-markup :rows))
            (row-width 0)
            (max-row-width 0))
        (seq-doseq (kbd-row rows)
          ;; TODO:
          )
        ))))
...
has `replyMarkupShowKeyboard' type."
  ...
  (if (let ((content (plist-get msg :content)))
        (and (eq 'messageInvoice (telega--tl-type content))
             (not (zerop (telega-tl-get0 content :receipt_message_id)))))
...
performance."
  ...
  (let* ((msg-ii (plist-get msg :interaction_info))
         (view-count (telega-tl-get0 msg-ii :view_count))
         (fwd-count (telega-tl-get0 msg-ii :forward_count))
...
(defun telega-ins--msg-comments (msg)
  ...
  (when-let ((msg-ri (telega-msg-match-p msg 'post-with-comments)))
    (let ((reply-count (telega-tl-get0 msg-ri :reply_count))
          (recent-repliers (plist-get msg-ri :recent_replier_ids)))
      (telega-ins--box-button2
          (telega-ins--as-string
           ...
           (if (zerop reply-count)
             ...
             (telega-ins (telega-i18n "lng_comments_open_count"
                         ...
                         (when (telega-msg-replies-has-unread-p msg)
                           "•"))))
          (telega-box-button-style 'comments)
...
(defun telega--fwd-info-source-chat (fwd-info)
  ...
  (let ((chat-id (telega-tl-get0 (plist-get fwd-info :source) :chat_id)))
    (unless (zerop chat-id)
...
(defun telega--fwd-info-action (fwd-info)
  ...
  (let* ((origin (plist-get fwd-info :origin))
         (origin-chat-id (telega-tl-get0 origin :chat_id))
         (origin-msg-id (telega-tl-get0 origin :message_id))
         (origin-sender-id (telega-tl-get0 origin :sender_user_id))
         (from-chat-id (telega-tl-get0 (plist-get fwd-info :source) :chat_id))
         (chat-id (if (not (zerop from-chat-id))
                      from-chat-id
                    origin-chat-id))
         (from-msg-id (telega-tl-get0 (plist-get fwd-info :source) :message_id))
         (msg-id (if (not (zerop from-msg-id))
                     from-msg-id
                   origin-msg-id)))
...
(defun telega-ins--msg-translation-inline (msg)
  "Insert message translation state."
  (when-let ((translated (plist-get msg :telega-translated)))
    (let* ((telega-palette-context 'translate)
           (palette (telega-msg-sender-palette (telega-user-me))))
      (telega-ins--line-wrap-prefix
          (propertize (telega-symbol 'vbar-left) 'face
        ...
        (telega-ins--raw-button
          ...
          (telega-ins--with-face
              (telega-face-with-palette 'telega-msg-inline-other
                  palette :foreground :background)
            (telega-ins (telega-symbol 'translate) " ")
            (if (and (not (plist-get translated :loading))
                     (not (telega-tl-str translated :text)))
                (telega-ins-i18n "lng_translate_box_error")
...
(defun telega-ins--msg-summary-inline (msg)
  "Insert AI summary state."
  (when-let ((summary (plist-get msg :telega-summary)))
    (let* ((telega-palette-context 'summary)
           (palette (telega-msg-sender-palette (telega-user-me))))
      (telega-ins--line-wrap-prefix
          (propertize (telega-symbol 'vbar-left) 'face
                      (telega-face-with-palette 'telega-msg-inline-other
                          palette :foreground :background))
        (telega-ins--raw-button
            (list :action #'telega-msg-summarize
                  'action 'telega-button--action
                  'help-echo "telega: Press to disable AI summary")
          (telega-ins--with-face
              (telega-face-with-palette 'telega-msg-inline-other
                  palette :foreground :background)
            (telega-ins (telega-symbol 'summarize-out) " "
                        (telega-i18n "lng_summarize_header_title"))
            (when-let ((lang-code
  ...
  (if (telega-msg-special-p msg)
    ...
    (let* ((chat (telega-msg-chat msg))
      ...
      (telega-ins--line-wrap-prefix (cons content-prefix content-wrap)
        ...
        (telega-ins--msg-summary-inline msg)
        (telega-ins--msg-translation-inline msg)
...
(defun telega-ins--message-ignored (msg)
  ...
  (cond ((plist-get msg :telega-ignored-revealed-p)
         (funcall telega-inserter-for-msg-button msg))

        ((functionp telega-ignored-messages-visible)
        ...
        (t
         (telega-ins--with-face 'telega-shadow
           (telega-ins "<" (if (stringp telega-ignored-messages-visible)
                               telega-ignored-messages-visible
                             "Ignored Message")
                       ">")))))
...
(defun telega-ins--root-bot-2lines (user)
  ...
  (let ((avatar (telega-msg-sender-avatar-image user)))
    ...
    (telega-ins--image avatar 1
                       :no-display-if (not telega-user-show-avatars))
    (telega-ins--with-face 'telega-shadow
      (let ((user-count (telega-tl-get0 (plist-get user :type)
                                        :active_user_count)))
...
(defun telega-ins--chopic-unread-trail (chopic)
  ...
  (let* ((unread (telega-tl-get0 chopic :unread_count))
         (mentions (telega-tl-get0 chopic :unread_mention_count))
         (reactions (telega-tl-get0 chopic :unread_reaction_count))
         (poll-votes (telega-tl-get0 chopic :unread_poll_vote_count))
...
(defun telega-ins--chat-verification-icon (chat)
  ...
  (let* ((info (telega-chat--info chat 'locally))
         ...
         (icon-ceid
          (telega-tl-get0 v-status :bot_verification_icon_custom_emoji_id 'int64)))
    ...
    (telega-ins--with-eliding (list :max title-width
      ...
      (telega-ins--with-face (when (telega-chat-secret-p chat)
        ...
        (telega-ins--msg-sender chat
          :with-title (plist-get fmt-plist :with-title)
...
Return non-nil if restrictions has been inserted."
  (when-let ((my-status (telega-chat-member-my-status chat)))
    (when (eq (telega--tl-type my-status) 'chatMemberStatusRestricted)
      (let* ((until (telega-tl-get0 my-status :restricted_until_date))
    ...
    (telega-ins--raw-button
      ...
      (telega-ins
        (telega-i18n "lng_group_requests_pending"
          :count (telega-tl-get0 jr-info :total_count))))
    t))

(defun telega-ins--chat-join-button (chat &optional bot-start-param)
  "Inserter for the CHAT's [Join] button."
  (when-let ((button-text
              (cond ((telega-chat-match-p chat '(type bot))
                     (cond ((telega-chat-match-p chat 'is-blocked)
                            (telega-i18n "lng_profile_restart_bot"))
                           ((not (telega-chat-match-p chat 'is-known))
                            (telega-i18n "lng_bot_start"))))

                    ((telega-chat-match-p chat 'is-blocked)
                     (telega-i18n "lng_unblock_button"))

                    ;; No need for button in these chats
                    ((or (telega-chat-match-p chat
                           '(or is-known (type private secret)))
    ...
    (telega-ins--box-button2
        (concat (upcase button-text)
                (when bot-start-param
                  (concat " " bot-start-param)))
        (telega-box-button-style 'chat-join)
      'action (lambda (_ignored)
                (cl-assert (not (telega-chat-secret-p chat)))
                (if (telega-chat-private-p chat)
                    (progn
                      (telega-msg-sender-unblock chat)
                      (when (telega-chat-bot-p chat)
                        (telega--sendBotStartMessage
                         (telega-chat-user chat) chat bot-start-param)
    ...
    (telega-ins--with-outline-palette palette
      (let* ((sponsor (plist-get sponsored-msg :sponsor))
        ...
        (telega-ins--with-face 'telega-msg-sponsored
          (let ((telega-photo-size-limits telega-sponsored-photo-size-limits))
...
MSG-REACTION is the `messageReaction' TDLib object."
  (let* ((reaction-type (plist-get msg-reaction :type))
    ...
    (telega-ins--with-style
        (telega-box-button-style
            (cond ((and reaction-chosen-p reaction-paid-p)
      ...
      ;; NOTE: Do not show number of reactions if less than recent
      ;; senders to resemble official client
      (when (> (telega-tl-get0 msg-reaction :total_count)
               (length (plist-get msg-reaction :recent_sender_ids)))
        (telega-ins--with-props '(face (:height 0.70) display (raise 0.15))
          (telega-ins (telega-number-human-readable
                       (telega-tl-get0 msg-reaction :total_count)))))
...
(defun telega-ins--saved-messages-tag (tag)
  ...
  (telega-ins--with-style (telega-box-button-style 'saved-messages-tag)
    ...
    (seq-doseq (msg-reaction reactions)
      ...
      (if-let ((tag (telega-saved-messages-find-tag
          ...
          (telega-ins--raw-button
              (list 'action (lambda (_button)
                              (transient-setup
                               'telega-transient-sm-tag-commands nil nil
                               :scope (list :tag tag :msg msg))))
...
BOT-MENU can be nil, in this case insert menu with bot commands."
  (cl-assert bot-menu)
  (let ((menu-url (plist-get bot-menu :url)))
    ...
    (telega-ins--box-button2
        (concat (telega-symbol 'bot-menu)
                (telega-tl-str bot-menu :text))
        (telega-box-button-style 'bot-menu)
