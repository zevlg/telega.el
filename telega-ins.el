...
If SLICE-NUM is specified, then insert N's."
  ;; NOTE: for svg images, we might take WxH from the spec
  (let* ((img-svg-p (eq 'svg (plist-get (cdr img) :type)))
         (img-w (when img-svg-p (plist-get (cdr img) :width)))
         (img-h (when img-svg-p (plist-get (cdr img) :height)))
         (img-size (if (and img-w img-h) (cons img-w img-h) (image-size img)))
  ...
  (telega-ins--one-lined
   (cl-case (telega--tl-type imc)
     ...
     (inputMessagePoll
      ...
      (telega-ins (plist-get imc :question))
